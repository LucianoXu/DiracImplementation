...
\section{Motivation}
\yx{One important idea: mentioning that registers are just like variables in classical programming language.}
...
\end{definition}
\yx{Here the ordered sorts are only for the purpose of expressing the theory. In \texttt{CiME2} and the implementation, there is only one universal sort.}
...
\yx{These part is not encoded in CiME2 code because the tool is already AC-extended.}
...
\yx{The biggest problem with variable set is that we cannot specify whether the term belongs to the set or not. Consider the example: $\sum_{i \in M} \delta_{i, x}$, this term reduces to $1$ when $x \in M$ and $0$ otherwise.}
...
\yx{
  But I found a speical critical pair:
  $$
  \left ( \sum_{i}   ( \langle  \mathit{i} | \cdot  \mathit{K0})  \left ( \mathit{X} \otimes  | \mathit{i} \rangle \right) \right )
  \reducefrom
  X \otimes \sum_{i} (\bra{i} \cdot \mathit{K0}). \ket{i}
  \reduce 
  X \otimes \mathit{K0}
  $$
  So I guess the completion rules should be further modified.
}
...
\yx{But maybe we want to rewrite in the opposite direction?}
...
\yx{We still have a very speical algorithm deciding a ``belonging to'' relation.}
...
\begin{align*}
  ...
  & A_{R} \cdot B_{R} \reduce (A \cdot B)_{R} \\
...
\yx{For a well-formed quantum register, the sub-register position is well-defined.}
...
\begin{align*}
  & \frac{R \textsf{ is in } Q}{A_{R} \cdot (B_{Q} \otimes X) \reduce (A_{R} \cdot B_{Q}) \otimes X} \\
  \\
  & \frac{Q \textsf{ is in } R}{(A_{R} \otimes X) \cdot B_{Q} \reduce (A_{R} \cdot B_{Q}) \otimes X} \\
  \\
  & A_{R} \cdot_I (B_{R} \otimes X) \reduce (A_{R} \cdot_I B_{R}) . X \\
  & (A_{R} \otimes X) \cdot_I B_{R} \reduce (A_{R} \cdot_I B_{Q}) . X \\
  & (A \cdot B) \cdot C \reduce A \cdot (B \cdot C) \\
  & (A + B) \cdot C \reduce (A \cdot C) + (B \cdot C) \\
  & C \cdot (A + B) \reduce (C \cdot A) + (C \cdot B) \\
  & \mathbf{1}_\mathcal{O} \cdot A \reduce A \\
  & A \cdot \mathbf{1}_\mathcal{O} \reduce A \\
  \\
  & \frac{R \textsf{ is in } S}{A_{Q; R} \cdot (B_{S; T} \otimes X) \reduce (A_{Q; R} \cdot B_{S; T}) \otimes X} \\
  \\
  & \frac{S \textsf{ is in } R}{(A_{Q; R} \otimes X) \cdot B_{S; T} \reduce (A_{Q; R} \cdot B_{S; T}) \otimes X} \\
\end{align*}