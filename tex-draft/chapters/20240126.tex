...
\begin{definition}[language of Dirac Notation]
  ...
  \begin{align*}
    ...
    & \mathcal{O} \textrm{(operator)} && O ::= x\ |\ \mathbf{0}_\mathcal{O}\ |\ K \otimes B\ |\ O^\dagger\ |\ S.O\ |\ O + O\ |\ O \cdot O\ |\ O \otimes O
  \end{align*}
  Here $x$ is a variable, $b$ is a ground term of $\Sigma_\mathcal{B}$ and $\alpha$ is a ground term of $\Sigma_\mathcal{S}$.

\end{definition}

\textbf{Remark: } Although we use the same symbol in different sorts ($B \cdot K$ and $O \cdot K$, for example), they are actually different funtions and can be easily distinguished by the context. This is also reflected in the \texttt{CiME} script. Therefore we avoid any possible ambiguity.
...
\begin{align*}
  & \vdash 0.K \reduce \textbf{0}_{\mathcal{K}}\\
  & \vdash 1.K \reduce K \\
...
\begin{align*}
  ...
  & \vdash K + K \reduce (1 + 1).K \\
  & \vdash S.K + K \reduce (S + 1).K \\
...
\begin{align*}
  ...
  & \vdash (K_1 \otimes B) \cdot K_2 \reduce (B \cdot K_2).K_1 \\
  & \vdash (O_1 \cdot O_2) \cdot K \reduce O_1 \cdot (O_2 \cdot K) \\
  & \vdash (O_1 \otimes O_2) \cdot ((O_1' \otimes O_2') \cdot K) \reduce ((O_1 \cdot O_1') \otimes (O_2 \cdot O_2')) \cdot K \\
...
\textbf{Remark: } The rules for bra are completely symmetric to the rules for ket, so we omit them here.
...
\begin{align*}
  & \vdash 0.O \reduce \mathbf{0}_\mathcal{O} \\
  & \vdash 1.O \reduce O \\
...
\begin{align*}
  ...
  & \vdash O + O \reduce (1 + 1).O \\
  & \vdash S.O + O \reduce (S + 1).O \\
