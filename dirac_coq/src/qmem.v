...
Proof. by rewrite !unitarylfE tf2f_adj tf2f_comp -(inj_eq (@tf2f_inj _ _ y y)) tf2f1. Qed.
...
Notation "''|' v ; x >" := (tket default_qmemory x v%VF).
Notation "''<' v ; x |" := (tbra default_qmemory x v%VF).
Notation "''|' u ; x >< v ; y |" := (muld (tket default_qmemory x u%VF) (tbra default_qmemory y v%VF)).
Notation "''|' u >< v ; x |" := (muld (tket default_qmemory x u%VF) (tbra default_qmemory x v%VF)).
Notation "''[' f ; x , y ]" := (tlin default_qmemory x y f%VF).
Notation "''[' f ; x ]" := (tlin default_qmemory x x f%VF).
Notation "''<' x | z ; w >" := (muld (tbra default_qmemory (tv2v w x%VF)) (tket default_qmemory (tv2v w z%VF))).
...
Notation "''<' x | z ; w >" := (muld (tbra w x%VF) (tket w z%VF)).
...
Lemma tketT_pairE T1 T2 (x : qreg (T1 * T2)) (u1 : 'Ht T1) (u2 : 'Ht T2) : 
  '| u1 ⊗t u2; x > = '| u1; x.1 > \⊗ '| u2; x.2 >.
Proof. by rewrite tketT_pair !eq_qreg_pair. Qed.  

Lemma tketT_tupleE n T (x : qreg T.[n]) (u : 'I_n -> 'Ht T) :
  '|tentv_tuple u; x > = \ten_i '|u i; x.[i] >.
Proof. by rewrite tketT_tuple !eq_qreg_tuple. Qed.

Lemma tketT_ffunE n (T : 'I_n -> qType) (x : qreg {qffun T})
   (u : forall i, 'Ht (T i)) :
...
Lemma tbraT_tuple n T (x : 'I_n -> qreg T) (u : 'I_n -> 'Ht T) :
  \ten_i '<u i; x i | = '<tentv_tuple u; (tuple: x) |.
Proof. by rewrite -tket_adj -tketT_tuple tketT_adj. Qed.

Lemma tbraT_ffun n (T : 'I_n -> qType) (x : forall i, qreg (T i))
   (u : forall i, 'Ht (T i)) :
  \ten_i '<u i; x i | = '<tentv_dffun u; (ffun: x) |.
Proof. by rewrite -tket_adj -tketT_ffun tketT_adj. Qed.

Lemma tbraT_pairE T1 T2 (x : qreg (T1 * T2)) (u1 : 'Ht T1) (u2 : 'Ht T2) : 
  '< u1 ⊗t u2; x | = '< u1; x.1 | \⊗ '< u2; x.2 |.
Proof. by rewrite tbraT_pair !eq_qreg_pair. Qed.  

Lemma tbraT_tupleE n T (x : qreg T.[n]) (u : 'I_n -> 'Ht T) :
  '<tentv_tuple u; x | = \ten_i '<u i; x.[i] |.
Proof. by rewrite tbraT_tuple !eq_qreg_tuple. Qed.

Lemma tbraT_ffunE n (T : 'I_n -> qType) (x : qreg {qffun T})
   (u : forall i, 'Ht (T i)) :
   '<tentv_dffun u; x | = \ten_i '<u i; x.-[i] |.
...
Proof. by rewrite tlinT_pairE tendC tlinT_pair. Qed.

Lemma tketTC T1 T2 (x : qreg (T1 * T2)) (v1 : 'Ht T1) (v2 : 'Ht T2) : 
  '| v1 ⊗t v2; x > = '| v2 ⊗t v1; (x.2,x.1)>.
Proof. by rewrite tketT_pairE tendC tketT_pair. Qed.

Lemma tbraTC T1 T2 (x : qreg (T1 * T2)) (v1 : 'Ht T1) (v2 : 'Ht T2) :
  '< v1 ⊗t v2; x | = '< v2 ⊗t v1; (x.2,x.1) |.
Proof. by rewrite tbraT_pairE tendC tbraT_pair. Qed.
...
Section tlinG.
Variable (T1 T2 T3 T4 : qType) (x1 : qreg T1) (x2 : qreg T2) (x3 : qreg T3) (x4 : qreg T4).
Variable (x12 : qreg (T1 * T2)) (x34 : qreg (T3 * T4)).
...
(***************************************************************************)
(* 
Lemma tlintGl : '[ N1; x1 ] \· '[ f, x1;x2 ] = '[(N1 ⊗f \1) \o f, x1;x2 ].
Proof. 
by rewrite -[RHS]tlinG -[X in _ = X \· _]tlinT -tf2f1 
  -dotd_ten/= -?dotdA/= ?dotIdid// ?fsubsetUr// -disjoint_neqE.
Qed.
Lemma tlintGr : '[ N2; x2 ] \· '[ f, x1;x2 ] = '[(\1 ⊗f N2) \o f, x1;x2 ].
Proof.
by rewrite -[RHS]tlinG -[X in _ = X \· _]tlinT -tf2f1 tendC 
  -dotd_ten/= -?dotdA/= ?dotIdid// ?fsubsetUl// fdisjoint_sym.
Qed.
Lemma tlinGtl : '[ f, x1;x2 ] \· '[ N1; x1 ] = '[ f \o (N1 ⊗f \1), x1;x2 ].
Proof.
by rewrite -[RHS]tlinG -[X in _ = _ \· X]tlinT -tf2f1 tendC
  -dotd_ten/= ?dotdA/= ?dotdIid// ?fsubsetUr// fdisjoint_sym.
Qed.
Lemma tlinGtr : '[ f, x1;x2 ] \· '[ N2; x2 ] = '[ f \o (\1 ⊗f N2), x1;x2 ].
Proof.
by rewrite -[RHS]tlinG -[X in _ = _ \· X]tlinT -tf2f1
  -dotd_ten/= ?dotdA/= ?dotdIid// ?fsubsetUl.
Qed.
Lemma tketGl : '[ N1; x1 ] \· '| v, x1;x2 > = '|(N1 ⊗f \1) v, x1;x2 >.
Proof. 
by rewrite -[RHS]tlin_ketG -[X in _ = X \· _]tlinT -tf2f1 
  -dotd_ten/= -?dotdA/= ?dotIdid// ?fsubsetUr.
Qed.
...
Lemma t1lin_braTl : [disjoint lb x & S] ->
  ('< v; x | ⊗ e) \· '[ f; x ] = '< f^A v, x | ⊗ e.
Proof. by move=>P; rewrite lind_braTl// tf2f_adj tf2f_apply. Qed.
Lemma t2lin_braTl : [disjoint lb x1 `|` lb x2 & S] ->
  ('< u, x1;x2 | ⊗ e) \· '[ N, x1;x2 ] = '<N^A u, x1;x2 | ⊗ e.
Proof. by move=>P; rewrite lind_braTl// tf2f2_adj tf2f2_apply. Qed.
Definition tlin_braTl := (t1lin_braTl, t2lin_braTl).

Lemma t1lin_braTr : [disjoint lb x & S] ->
  (e ⊗ '< v; x |) \· '[ f; x ] = e ⊗ '< f^A v, x |.
Proof. by move=>P; rewrite lind_braTr// tf2f_adj tf2f_apply. Qed.
Lemma t2lin_braTr : [disjoint lb x1 `|` lb x2 & S] ->
  (e ⊗ '< u, x1;x2 |) \· '[ N, x1;x2 ] = e ⊗ '< N^A u, x1;x2 |.
Proof. by move=>P; rewrite lind_braTr// tf2f2_adj tf2f2_apply. Qed.
...
Section ketBT.
Variable (I : Type) (r : seq I) (P : pred I) (d : I -> qvars T) (F G : I -> 'Hs T).
...
Lemma touterMBT : (\ten_(i <- r | P i) '|F i; d i>) ∘ (\ten_(i <- r | P i) '<G i; d i|) 
