...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item \textbf{critical pairs containing} \textsc{(Squash1)}/\textsc{(Squash2)} 
      ...
      \begin{flalign*}
        & \bra{(\utt, s)} \cdot (a \otimes b) \reduce \left \{
          \begin{aligned}
            ...
            & \textsc{(ReFacBPair)} && (\bra{\utt} \cdot a) \otimes (\bra{s} \cdot b) \reduce (\mathbf{1} \cdot a) \otimes (\bra{s} \cdot b)
          \end{aligned}
      ...
      \begin{flalign*}
        & (\bra{\fst\ (e_1, e_2)} \cdot a) \otimes (\bra{\snd\ (e_1, e_2)} \cdot b) \reduce \left \{
          \begin{aligned}
            & \textsc{(Proj1)} && (\bra{e_1} \cdot a) \otimes (\bra{\snd\ (e_1, e_2)} \cdot b) \\
            & \textsc{(TsrFacBra)} && \bra{(e_1, e_2)} \cdot (a \otimes b)
          \end{aligned}
        \right \} \\
        & \qquad \qquad \reduce (\bra{e_1} \cdot a) \otimes (\bra{e_2} \cdot b) &
      \end{flalign*}
      \textbf{Remark:} Similar for 
      \begin{itemize}
      ...
      \begin{flalign*}
        & \bra{(\fst\ e, \snd\ e)} \cdot (a \otimes b) \reduce \left \{
          \begin{aligned}
            & \textsc{(Pair)} && \\
            & \textsc{(ReFacBPair)} && (\bra{\fst\ e} \cdot a) \otimes (\bra{\snd\ e} \cdot b)
          \end{aligned}
        \right \} \reduce \bra{e} \cdot (a \otimes b) &
      \end{flalign*}
    ...
    \item \textbf{critical pairs containing} \textsc{(Fac2)}/\textsc{(Fac1)}/\textsc{(Fac0)}
      ...
      \end{flalign*}     
        ...
        \item \textbf{critical pairs containing} \textsc{(MulZeroL)}/\textsc{(MulZeroR)}
          ...
          \begin{flalign*}
            & (\bra{\fst\ e} \cdot \mathbf{0}_{\tau, \sigma}) \otimes (\bra{\snd\ e} \cdot b) \reduce \left \{
              \begin{aligned}
                & \textsc{(MulZeroR)} && \mathbf{0} \otimes (\bra{\snd\ e} \cdot b)\\
                & \textsc{(TsrFacBra)} && \bra{e} \cdot (\mathbf{0} \otimes b)
              \end{aligned}
              \right \} \reduce \mathbf{0} &
          \end{flalign*}
        ...
        \begin{flalign*}
          ...
          \begin{flalign*}
            & (\bra{\fst\ e} \cdot \ket{s}) \otimes (\bra{\snd\ e} \cdot b) \reduce \left \{
              \begin{aligned}
                & \textsc{(MulBraKet)} && \delta_{\fst\ e, s} \otimes (\bra{\snd\ e} \cdot b) \\
                & \textsc{(TsrFacBra)} && \bra{e} \cdot (\ket{s} \otimes b)
              \end{aligned}
              \right \} \reduce & %%%%% REFACBPAIR rule should be changed
          \end{flalign*}
          ...
          \begin{flalign*}
            & (a_1 \otimes a_2) \cdot (b_1 \otimes b_2) \reduce \left \{
              ...
              \end{aligned}
            \right \} \reduce \text{(sorted)}\\
            & (\Gamma \vdash a_1 : [\unit, \tau_1], \Gamma \vdash a_2 : [\unit, \tau_2], \Gamma \vdash b_1 : [\sigma_1, \unit], \Gamma \vdash b_2 : [\sigma_2, \unit]) &
          \end{flalign*}
          \yx{We have to prove that the two expressions will result in the same sorted normal form.}

          \textbf{Remark:} Similar for other critical pairs among \textsc{(ReFac2)}, \textsc{(ReFac1)} and \textsc{(ReFac0)}.


          \begin{flalign*}
            & \bra{(\utt, s)} \cdot (e \otimes b) \reduce \left \{
              \begin{aligned}
                & \textsc{(ReFac1)} && e \otimes (\bra{(\utt, s)} \cdot b) \\
                & \textsc{(ReFacBPair)} && (\bra{\utt} \cdot e) \otimes (\bra{s} \cdot b) \reduce (\textbf{1} \cdot e) \otimes (\bra{s} \cdot b)
              \end{aligned}
            \right \} \reduce e \otimes (\bra{s} \cdot b) \\
