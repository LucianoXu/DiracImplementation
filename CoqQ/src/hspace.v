...
Require Import mcextra notation hermitian.
...
Lemma mxrank_trmxC m n (A : 'M[T]_(m,n)) :
  \rank (A^*t) = \rank A.
Proof. by rewrite adjmxTC mxrank_conj mxrank_tr. Qed.
...
Proof. by rewrite /lfrank/supplf h2mx_comp/= -[RHS]suppmx_rank !mx2hK. Qed.

Lemma cosupplf_rank G (A : 'Hom(H,G)) : \Rank (cosupplf A) = \Rank A.
Proof. by rewrite /lfrank/cosupplf h2mx_comp/= -[RHS]cosuppmx_rank !mx2hK. Qed.

Lemma suppvlf G (A : 'Hom(H,G)) : A \o supplf A = A.
Proof. apply/h2mx_inj; rewrite/supplf !h2mx_comp !mx2hK; exact: suppmx_id. Qed.

Lemma cosupplfv G (A : 'Hom(H,G)) : cosupplf A \o A = A.
Proof. apply/h2mx_inj; rewrite/cosupplf !h2mx_comp !mx2hK; exact: cosuppmx_id. Qed.

Lemma supplf_proj G (A : 'Hom(H,G)) : supplf A \is projlf.
Proof. rewrite qualifE /supplf h2mx_comp/= !mx2hK; exact: suppmx_proj. Qed.
...
Proof. rewrite qualifE /cosupplf h2mx_comp/= !mx2hK; exact: cosuppmx_proj. Qed.
...
rewrite h2cE=>->; rewrite trmx_mul [h2vU *m _]mulmxA.
...
by move=>/submxP[D]->; rewrite !trmx_mul !trmxK ![h2vU *m _]mulmxA mulmxV 
  ?h2vU_unitmx// mul1mx -/(cosuppmx _) mulmxA P.
move=>/eqP P1; apply/submxP; exists (h2vU *m (v2r x)^T)^T.
apply/trmx_inj/(unitmx_mulmxI h2vU_unitmx).
by rewrite -{1}P1 !trmx_mul !trmxK ![h2vU *m (_ *m _)]mulmxA mulmxV ?h2vU_unitmx// mul1mx.
Qed.
...
by rewrite -adj_dotEr hermf_adjE/= -comp_lfunE projf_idem projfE.
Qed.
...
by rewrite !(hs2vsK, addv2h, capv2h).
Qed.
...
by rewrite /outp unlock/outp_def/= mx2hK c2hK !adjmxE tr_col map_row.
Qed.
...
Lemma ranklfMfreel (A : 'Hom(F,G)) (B : 'Hom(H,F)) :
  (Vector.dim F <= \Rank B)%N -> \Rank (A \o B) = \Rank A.
Proof.
