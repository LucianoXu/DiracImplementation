...
Ltac move_ord := repeat match goal with
  | [ |- _ -> _ ] => intro; rewrite ? AutoNatTheory.R4 /= ? AutoNatTheory.Qord /=
  | [ |- forall _, _ ] => intro; rewrite ? AutoNatTheory.R4 /= ? AutoNatTheory.Qord /=
  end.
...
Ltac elim_odd :=
  ...
  | [o := ssrnat.odd ?x |- _ ] => 
    case E: o=>/=;
    [ (try move=>/AutoNatTheory.Q0 []); move: E=>/AutoNatTheory.R45[t Pt]; rewrite ?(@AutoNatTheory.R44 _ _ Pt)/= ?Pt
    | (try move=>/AutoNatTheory.Q0 []); move: E=>/AutoNatTheory.R46[t Pt]; rewrite ?(@AutoNatTheory.R43 _ _ Pt)/= ?Pt
    ];
...
Ltac elim_div := 
  ...
  | [q := div.divn ?x (S ?y) |- _ ] => 
    ...
    move: q r; intros ? ?; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /=
  | [t := div.divn ?x ?y |- _ ] => 
    ...
    move: y_t q; case; 
    [ move => q; rewrite ? AutoNatTheory.Q13 ? AutoNatTheory.Q14 => ->; clear q 
    | let Eq := fresh "_quotientE_" in
      ...
      move: r Ey; clear Eq; intros ?; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /=
    ]
  end.
...
Ltac elim_mod := 
  ...
  | [r := div.modn ?x (S ?y) |- _ ] => 
    ...
    move: r q; intros ? ?; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /=
  | [r := div.modn ?x ?y |- _ ] => 
    let y_t := fresh "divisor_" in
    let Ey := fresh "_divisorE_" in
    set y_t := y in r *;
    ...
    move: y_t r; case; 
    [ move => r; rewrite ? AutoNatTheory.Q13 ? AutoNatTheory.Q14 => ->; clear r
    | let Er := fresh "_remainderE_" in
      ...
      move: (div.divn_eq x (S y_t)); rewrite - {1} [x] /x_t => divP;
      rewrite ? divP /x_t; clear x_t divP;
      (have : lt (div.modn x (S y_t)) (S y_t) by apply/ssrnat.ltP/div.ltn_pmod);
      move: (div.divn_eq x (S y_t)); rewrite - ? Er - ? /q;
      move: q Ey; clear Er; intros ?; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /=
    ]
  end.
...
Ltac move2 := repeat match goal with
  ...
  | [ |- forall _ : ordinal _, _ ] => 
      ...
      let x := fresh "n" in
      let h := fresh "H" in
      case => x ; rewrite ? (AutoNatTheory.R4,AutoNatTheory.Qord) /= => h; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /=;
      move: h {+} h => /ssrnat.ltP; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /=; (try move => + _ );
      (try do ? elim_odd); (try do ? elim_div); (try do ? elim_mod); (try do ? elim_bump)
  | [ |- forall _ : nat, _ ] => 
      (* idtac "move nat"; *)
      let x := fresh "n" in 
  ...
  | [ |- is_true (leq (S _) _) -> _ ] => let x := fresh "H" in 
      move => /= /ssrnat.ltP x; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /= in x
  | [ |- is_true (leq _ _) -> _ ] => let x := fresh "H" in 
      move => /= /ssrnat.leP x; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /= in x
  | [ |- is_true (_ == _) -> _ ] => let x := fresh "H" in 
      move => /= /eqP x; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /= in x
  | [ |- is_true (_ != _) -> _ ] => let x := fresh "H" in 
      move => /= /eqP x; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /= in x
  | [ |- _ -> _ ] => 
      (* idtac "move prop"; *)
      let x := fresh "H" in
      move => /= x; rewrite /= ? (AutoNatTheory.R4, AutoNatTheory.Qord) /= ? (AutoNatTheory.Qm, AutoNatTheory.Qnat) in x
  | [ |- forall _, _ ] => 
      (* idtac "move var"; *)
      let x := fresh "x" in
      move => /= x /=; rewrite ? (AutoNatTheory.R4, AutoNatTheory.Qord) /=; 
      (try do ? elim_odd); (try do ? elim_div); (try do ? elim_mod); (try do ? elim_bump)
  | [ |- _ ] => rewrite /= ? (AutoNatTheory.R4, AutoNatTheory.Qord) /= ? (AutoNatTheory.Qm, AutoNatTheory.Qnat)
end.

Ltac mc_nat_convert := move_all; move_ord; move_all; move2; move_all; rewrite ?AutoNatTheory.Rm.
Ltac mc_nat := move_all; move_ord; move_all; move2; move_all; rewrite ?AutoNatTheory.Rm; Lia.nia.
