...
Require Import mcextra mcaextra notation mxpred ctopology tensor.
Require Import hermitian quantum inhabited hspace qtype.
...
Variable (pt st : bool) (n : nat) (fx : 'I_n -> (vars bool)).
...
Proof.
rewrite pvarsEt dotd_mul/=tensM//= =>[i j _ _ |]; first by apply: disx.
...
rewrite/=dotd_mul/= tensM//==>[+ + _ _|]; first exact: disx.
...
Variable (x : 'I_n -> (vars T)).
Hypotheses (disx : forall i j, i != j -> [disjoint lb (x i) & lb (x j)]).
...
Definition rev_circuit :=
  \big[seqc/skip]_(i < n./2) 
    [ut [x (half_ord i) , x (rev_ord (half_ord i))] := SWAP ].

Lemma rev_disjoint (i j : 'I_n./2) : i != j -> 
  [disjoint lb (x (half_ord i)) :|: lb (x (rev_ord (half_ord i))) & 
    lb (x (half_ord j)) :|: lb (x (rev_ord (half_ord j)))].
Proof.
move=>nij; rewrite disjointUX !disjointXU !disx ?(can_eq rev_ordK)//.
by rewrite half_ord_rev_neq. by rewrite eq_sym half_ord_rev_neq.
Qed.
...
rewrite -(t1ket_sum (pvars_tuple rev_disx)) dotd_sumr; apply eq_bigr=>i _.
rewrite -!tketZ dotdZr; f_equal; rewrite !pvarsEt.
move: rev_disjoint. case: n x disx t i; clear n x disx.
by move=>x disx t i; rewrite !big_ord0 bigd dot1d.
move=>n x disx t i rd; rewrite !big_half_split !bigdE/=; case E: (odd n).
2: rewrite dotdTll/=; last rewrite frevE uphalf_ord_odd_rev ?E//; f_equal.
1,4: rewrite ?tend1 dotd_mul/= tensM//= ?bigd =>[?? _ _/rd//|];
apply eq_bigr=>j _; rewrite tketT tlin_ketM ?disx ?half_ord_rev_neq// swaptfEtv;
by rewrite tendC !frevE rev_ordK tketT.
all: rewrite disjoint_sym; apply/bigcup_disjoint_seqP=>j/andP[] _ Pj;
rewrite disjointXU !disx//; last rewrite -(can_eq rev_ordK) rev_ordK uphalf_ord_odd_rev ?E//;
by rewrite -(inj_eq val_inj)/= eq_sym ltn_neq.
Qed.
...
2: by move=>i j _ _; apply: rev_disjoint.
rewrite -rev_unitaryE; do 2 f_equal; apply eq_bigr=>i Pi;
by rewrite tf2f2cE// disx// half_ord_rev_neq.
Qed.
...
by rewrite tf2f2cE// ?tlin_adj ?swaptf_adj// disx// half_ord_rev_neq.
Qed.
...
Definition QFT_sub (x : vars bool) n (s : 'I_n -> (vars bool)) :=
    [for i do 
        [ut [x , s i] := BControl (PhGate (omegan i))]
    ].

Definition QFT_iter n (s : 'I_n -> (vars bool)) :=
  [for i do 
    [ut s i := ''H] ;; (QFT_sub (s i) (fright s i))
  ].

Definition QFT_cir n (s : n.-tuple (vars bool)) :=
  ((QFT_iter s) ;; (rev_circuit s)).

Lemma QFT_sub_cast x n (s : 'I_n -> (vars bool)) m (eqn : n = m) :
  QFT_sub x (cast_fun_ord eqn s) = QFT_sub x s.
Proof. by case: m / eqn. Qed.
...
Lemma QFT_sub_rcons n x y (s : 'I_n -> (vars bool)) :
    (QFT_sub x (frcons s y)) =s 
    (QFT_sub x s ;; [ut [x,y] := BControl (PhGate (omegan n))]).
Proof.
rewrite /eqcmd /QFT_sub/= fsemE !fsem_big/= big_ord_recr/= comp_soElr.
f_equal; last by rewrite frcons_max.
by apply eq_bigr=>i _; rewrite widen_lift frconsE.
Qed.

Lemma QFT_iter_no_while n (s : 'I_n -> (vars bool)) : no_while (QFT_iter s).
Proof. by rewrite no_while_for// =>i _ /=; rewrite no_while_for. Qed.
...
Lemma RS_QFT_sub n (x : vars bool) (b : bool) (s : 'I_n -> (vars bool)) 
  (sb : n.-tuple bool) :
    (forall i, [disjoint lb x & lb (s i)]) ->
    (forall i j, i != j -> [disjoint lb (s i) & lb (s j)]) ->
    let: prev := '|''b, x> \⊗ (\ten_i '|''(sb~_i), s i>) in
    let: postv :=  '|'ph (bitstr2rat (b :: sb)), x> \⊗ (\ten_i '|''(sb~_i), s i>) in
    ⊨s [pt,st] { prev  } ([ut x := ''H] ;; (QFT_sub x s)) { postv }.
Proof.
elim: n s sb=>[s sb _ _|n IH s bs xs us]; last first.
case/frconsP: s xs us=>sh sl /=/rcons_disjointx[]P1 P2/rcons_disjoint[]P3 P4.
case/tupleNP: bs=>bsl bsh/=.
rewrite big_ord_recr/= bigdE.
rewrite !tnthN frcons_max. under eq_bigr do rewrite tnthNS widen_lift frconsE.
rewrite [SPRE]tendA [SPOST]tendA.
move: P1; rewrite QFT_sub_rcons fsem_seqA=>P1.
apply (RS_SC ('|'ph (bitstr2rat (b :: bsh)), x> 
  \⊗ (\ten_i '|''(bsh~_i), sh i>) \⊗ '|''bsl, sl>))=>/=; last first.
apply: RV_UT=>/=; rewrite [LHS]tendAC [in RHS]tendAC dotdTll/==>[||].
3: by f_equal; rewrite test1 ?size_tuple// tf2f2cE// tf2f2_adj.
3: move: (IH sh bsh P2 P4); apply: RV_Frame.
3: by rewrite/=/QFT_sub no_while_for//=; clear -pt; case: pt.
1-4: rewrite ?setUid /=?fvars_QFT_sub -?bigcup_add disjointUX ?Q4/=.
3,4: rewrite P1/= disjoint_sym ?big_tuple. 1,2: apply/andP; split.
1-6: by apply/bigcup_disjoint_seqP=>i _.
rewrite /QFT_sub bigd !big_ord0 !tend1.
...
rewrite tuple0/= [in _ [::b]]unlock.
by case: b; rewrite !sign_simp ?mul0r addr0 ?mulr0 ?mulfV// ?expip0 ?expip1 !sign_simp.
Qed.

Lemma QFTbv_rev n (s : 'I_n -> (vars bool)) (sb : n.-tuple bool) 
  (diss : forall i j, i != j -> [disjoint lb (s i) & lb (s j)]) :
...
rewrite [in RHS](reindex_inj rev_ord_inj)/= bigd; apply eq_bigr=>i _; congr ('|_,_>).
by rewrite frevE.
Qed.

Lemma RS_QFT_iter n (s : 'I_n -> (vars bool)) (sb : n.-tuple bool) 
  (diss : forall i j, i != j -> [disjoint lb (s i) & lb (s j)]) :
...
elim: n s sb diss =>[s sb _ | n IH s sb us];
  first by rewrite /QFT_iter/= !big_ord0 bigd/=; apply: AxS_Sk.
rewrite !big_ord_recl/= !bigdE.
case/fconsP: s us=>x bs; 
case/tupleP: sb=>b bsb/cons_disjoint[] P2 P3.
rewrite !tnth0 fcons0; under eq_bigr do rewrite !tnthS fconsE.
rewrite QFT_iterE.
(* under [in SPOST]eq_bigr do rewrite fconsE. *)
apply (RS_SC ('|'ph (bitstr2rat (b :: bsb)), x>
  \⊗ (\ten_i '|''(bsb ~_ i), bs i>)))=>/=; last first.
apply: RV_Framel=>/=; last by under [in SPOST]eq_bigr do rewrite fconsE; apply IH.
clear -bs; case: pt=>//; apply: QFT_iter_no_while.
3 : by apply RS_QFT_sub. rewrite fvars_QFT_iter. 
2: rewrite setUC; under eq_bigr do rewrite fconsE.
all: by rewrite 1?setUid disjoint_sym ?big_tuple; apply/bigcup_disjoint_seqP=>i _.
Qed.

Lemma RS_QFT_cir n (s : 'I_n -> (vars bool)) (sb : n.-tuple bool) 
  (diss : forall i j, i != j -> [disjoint lb (s i) & lb (s j)]) :
...
rewrite adjfK VUnitaryE/= uniformtvE card_ord -tketZ tendZl /vv -tket_sum -tket_sum tend_sumr.
+ apply: (RS_SC _ _ (AxV_UT _))=>/=.
rewrite tf2f2c_adj adjfK [in SPOST]tf2f2cE// dotdZr dotd_sumr.
under eq_bigr do rewrite tend_suml dotd_sumr/=.
suff P1: [disjoint lb p :|: lb q & lb r].
under eq_bigr do under eq_bigr do rewrite -tketT tendA tketT (t2lin_ketTl npq _ _ _ P1)/=.
+ apply: (RS_SC _ _ (AxV_UT _))=>/=.
+ apply: (RS_forward (R := \sum_j('|''(δ j) ⊗t v (δ j), p;r> \⊗ '|β j *: u j, q>))).
rewrite linear_sum/= dotd_sumr; apply eq_bigr=>i _.
rewrite tf2f_adj -/IQFT -tketT tendAC -tendA.
under eq_bigr do rewrite 2!linearZ/= MultiplexerEt expmxipEt -tentvZS -linearZr/= -tketT -tendA.
rewrite -tend_suml -tendZl tket_sum tketZ tlin_ketTl/=.
- by rewrite disjointXU npq npr.
congr ('|_,p>\⊗ _).
...
+ apply: (RS_SC _ _ (AxV_UT _))=>/=.
rewrite tf2f2cE// tf2f2_adj dotd_sumr.
suff P2: [disjoint lb p :|: lb r & lb q].
under eq_bigr do rewrite (t2lin_ketTl npr _ _ _ P2)/= PUnitaryEV -tketT tendAC.
rewrite -tend_suml.
...
by rewrite -tend_suml -tendZl tket_sum tketZ -QFTvE tlin_ketTl -?disjoint_neqE//= IQFTEt.
...
rewrite !tlin_linTll//= ?tlin_linTrl//=  !outp_comp !onb_dot !eqxx 
  !scale1r !outp_comp !onb_dot !eqxx !scale1r !['[_,r] \⊗ _]tendC -!tendA -tendDr.
rewrite /R adjdT !ket_adj tend_mul ?disjoint0X// !touter.
apply: le_wptend2l=>/=. by rewrite disjointXU npq npr.
...
Lemma R_HHL_loop_P :
  ...
  { Q }.
apply: tbR_LP_P=>/=.
1,2: rewrite tlinT tend_correct dlin_id -tenf11; apply: lev_pbreg2.
1,6: by rewrite disjointUX npr nqr.
1,5: rewrite tf2f2_ge0//. 4,7: rewrite tf2f2_le1//.
1,2,3,7: apply: psdf_ge0.
1,2,3,4: apply: obsf_le1.
apply: (R_SC [SqrD of P' ]).
rewrite /=/P' -!tendA [POST]tendC -tendA.
rewrite tendC -tendA -tlin1 tendC R_El/= 1?disjoint_sym.
2: apply: tAx_InF.
...
by rewrite /P'/= !adjdT !ket_adj !tend_mul/= ?disjointX0// !touterM.
Qed.


Lemma R_HHL_P : ⊨ps { :1 } HHL { '|x, q>}.
Proof.
rewrite stateE adjd1 dnumM mul1r ket_adj touterM.
apply/(R_Er (W:= lb p :|: lb r) _)=>/=.
by rewrite disjointXU nqr disjoint_sym npq.
apply: (R_Orr (Q:=Q) _)=>/=.
by rewrite /Q -tendA tendCA tlinT//; apply: le_wptend2l=>/=;
...
rewrite /P' tendC; apply: (R_SC _ _ (tAx_In _))=>/=.
rewrite outpE ns_dot scale1r ns_dot scale1r.
apply: (R_SC _ (tAx_InF _))=>/=;
rewrite ?disjointX0// tend1 tendC; apply: tAx_InF.
1,2: by rewrite ?disjointXU disjoint_sym ?npq// npr/= disjoint_sym.
apply: R_Or=>//=. rewrite -[P']add0r levD//.
all: rewrite tendC dformE tlin_adj adj_outp tlin_linTll// tlin_linTrl//=.
all: rewrite outp_comp ns_dot scale1r outp_comp ns_dot scale1r /P'.
2: rewrite tendC. all: rewrite !tendA tlinT !tend_correct ?lin_lef.
1: rewrite lin_gef0.
...
rewrite ?disjoint0X//=tend1 -pvarsEt_nseq.
...
rewrite !dotdZr; f_equal; rewrite/= pvarsEt (bigD1 i)//= bigdE tlin_ketTl/=; last first.
...
rewrite pvarsEt (bigD1 i.1)//= bigdE (bigD1 i.2) 1?eq_sym ?H2// bigdE tendA  
...
under eq_bigr do rewrite dotdZr (ParaHadamard_tuple disx) linear_sum/=.
...
Lemma unitaryf_sym (V : chsType) (U : 'FU(V)) u v : (U^A u == v)%VF = (U v == u).
Proof.
...
apply: (RS_SC _ _ (AxV_UT _)).
rewrite/= tf2f_adj tlin_ket -[SPOST]tend1.
...
rewrite adjdZ muldZl muldZr conjC_real scalerA dnum_adj dnumM conjC1 mulr1 dnumZ mulr1
...
under eq_bigr do rewrite tentvZl linearZ/= MultiplexerEVt /Uf_fun eigUn tentvZr 
  scalerA -tketZ dotdZr tf2f_adj -tketT (t1lin_ketTl _ _ _ nxy)/= -tendZl tketZ.
...
rewrite tend1 -pvarsEt_cst -pvarsEf -/i0 tendC QFT_FG_apply (tketT px) tketGl//.
apply: (RS_SC _ (AxV_UTF _))=>//=.
...
move: (RS_HSP true true); rewrite stateE dnum_adj dnumM conjC1 mulr1 ket_adj touterM=>IH.
apply: (R_back _ (tR_PInnerl disy ''i _ IH)); last by rewrite vf_norm1.
rewrite big1 ?dnum0// =>j _; rewrite /vf linearZ/= linear_sumr/= big1=>[k nk|].
...
Lemma R_HSP_in pt st i : 
  i \in HC -> ⊨ [pt,st] { ((#|HC|%:R)^-1)%R%:D } HSP { '[ [> ''i ; ''i <] , px ] }.
Proof.
move=>Pi; apply/no_while_enough=>//.
move: (RS_HSP true true); rewrite stateE dnum_adj dnumM conjC1 mulr1 ket_adj touterM=>IH.
apply: (R_back _ (tR_PInnerl disy ''i _ IH)); last by rewrite vf_norm1.
...
rewrite /vf dotpZl dotp_suml big1 ?mulr0 ?normr0 ?dnum0//  =>j Pj.
...
Lemma RS_HSP_in pt st i (t : {set G}): 
  i \in HC -> t \in [set rcoset H x0 | x0 : G] ->
   ⊨s [pt,st] { (#|H|%:R / #|G|%:R)%:D } HSP { '|''i ⊗t ''(t0 + ff (repr t)), px;y >}.
Proof.
