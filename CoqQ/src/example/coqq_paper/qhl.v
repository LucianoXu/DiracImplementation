...
Require Import mcextra mcaextra notation hermitian prodvect tensor mxpred.
Require Import extnum ctopology quantum hspace inhabited.
...
Import Vector.InternalTheory CTopology HermitianTopology.Exports.
...
elim: i=>[|i]. by rewrite /cg1/cg/cf/f/R/= dualqmE lerBlDl soE (ifso_bool b)
...
rewrite -(qm_tpE [QM of liftf_fun M]) (big_boolb b)/= levD=>[||//];
...
Lemma eq_lim (T : Type) (T' : topologicalType) (F : set (set T)) (f g : T -> T') :
...
have v2o : [< v2 ; u >] = 0.
move: (memh_proj (~` <[u]>)%HS v). rewrite memhCE hsCK -/v2 hline_def hsE/= 
...
by rewrite !dlin_id. 1,2,3: rewrite ?dlin_id !linear0 !diracE;
rewrite /local_hoare ?cplmt0 ?liftf_lf1 ?linear0//.
move=>H1. exists S. exists T. rewrite !dlin_id; do !split=>//; apply/sqrdP.
Qed.
Global Arguments dirac_localP [pt st S T P c].

Lemma dirac_sqrP pt st S T (P : 'D_S) (Q : 'D_T) c :
  ⊨ [pt,st] { P } c { Q } <-> ⊨l [pt,st] { P S S } c { Q T T }.
Proof. by rewrite -dirac_localP {1}sqrd_linE {1}(sqrd_linE Q). Qed.
...
Lemma Ax_Sk pt st S (P : 'D_S) :
  ⊨ [pt,st] { P } skip_ { P }.
Proof. by rewrite dirac_sqrP sqrd_linE dlin_id; apply Ax_Sk_L. Qed.
Global Arguments Ax_Sk [pt st S].

Lemma Ax_UT pt st S T (U : 'FU[H]_S) (P : 'D_T) :
  ⊨ [pt,st] { '[U^A] \· P \· '[U]} (unitary_ U) { P }.
Proof.
rewrite dirac_sqrP/= sqrd_linE/= !sdotd_correct !dlin_id; apply Ax_UT_L.
Qed.
Global Arguments Ax_UT [pt st S T U].

Lemma R_UT pt st (Q : 'D) S W (U : 'FU[H]_S) (P : 'D_W) :
  Q = '[U^A] \· P \· '[U] -> ⊨ [pt,st] { Q } (unitary_ U) { P }.
Proof. by move=>/esym P1; apply/(R_back P1)/Ax_UT. Qed.
Global Arguments R_UT [pt st Q S W U P].

Lemma Ax_UTF pt st S T (U : 'FU[H]_S) (P : 'D_T) :
  ⊨ [pt,st] { P } (unitary_ U) {  '[U] \· P \· '[U^A] }.
Proof.
rewrite dirac_sqrP/= sqrd_linE/= !sdotd_correct !dlin_id; apply Ax_UTF_L.
Qed.
Global Arguments Ax_UTF [pt st S T U].

Lemma R_UTF pt st (Q : 'D) S W (U : 'FU[H]_S) (P : 'D_W) :
...
Proof.
move=>P1; symmetry; rewrite sqrd_linE tend_correct; split;
  move=>[S' [T]][/sqr_linP/=/orP[/eqP<-|/eqP P2]]; rewrite ?dlin_id=>[[Pt P3]].
1,2: exists (S :|: W). 3,4: exists S. 
all: exists T; (do 2 ?(split=>//; do 1?apply/sqrdP)); rewrite !dlin_id.
...
Proof.
move=>P1; symmetry; rewrite sqrd_linE tend_correct; split;
  move=>[S [T']][Ps][/sqr_linP/=/orP[/eqP<-|/eqP P2]]; rewrite ?dlin_id=>P3.
all: exists S. 1,2: exists (T :|: W). 3,4: exists T. 
all: (do 2 ?(split=>//; do 1?apply/sqrdP)); rewrite !dlin_id.
...
Proof.
move=>[S [T]][Ps [Pt P1]].
rewrite (sqrdE Pt) sqrd_linE=>[[T' [W [/sqr_linP/=/orP[/eqP P2|/eqP P3] [Pw]]]]].
  case: T' / P2; rewrite dlin_id=>P2; exists S; exists W; 
  by do 2 split=>//; move: P1 P2; exact: R_SC_L.
move: P1; rewrite P3 linear0 diracE=>P1 P2; exists S; exists W; 
...
Lemma R_Frame pt st W (R : 'D_W) S (P : 'D_S) T (Q : 'D_T)  c :
  (pt || no_while c) ->
  (0 : 'F_W) ⊑ R W W -> 
    [disjoint (fvars c) & W] -> [disjoint S :|: T & W] ->
...
by rewrite -dirac_localP -!tend_correct -!sqrd_linE.
Qed.
Global Arguments R_Frame [pt st W R S P T Q c].

Lemma R_Framel pt st W (R : 'D_W) S (P : 'D_S) T (Q : 'D_T) c :
  (pt || no_while c) ->
  (0 : 'F_W) ⊑ R W W -> 
  [disjoint (fvars c) & W] -> [disjoint S :|: T & W] -> 
...
(* initialization *)
Lemma Ax_In_onb pt st S (v : 'NS_S) (F : finType) (onb : 'QONB_(F;S)) T (P: 'D_T) : 
  ⊨ [pt,st] { \sum_i (dform ('| v > \o (onb i)^A) P) } (init_ v) { P }.
Proof.
move: (@Ax_In_L pt st _ v _ [ONB of (d2onb onb)] _ (P T T))=>/=.
rewrite -dirac_localP -sqrd_linE; apply/R_back.
rewrite linear_sum/=; apply eq_bigr=>i _.
by rewrite -!sdotd_correct /dform -sqrd_linE -!outerM adjdM adjdK -!ket_adj !d2vK/=.
Qed.
Global Arguments Ax_In_onb [pt st S v F] onb [T].

Lemma Ax_InF pt st W S (v : 'NS[H]_S) : 
  ⊨ [pt,st] { \1_W } (init_ v) { '[ [> v ; v <] ] }.
Proof. by apply/dirac_sqrP=>/=; rewrite !dlin_id; exact: Ax_InF_L. Qed.
...
Lemma R_IF pt st (F: finType) S (M : 'QM[H]_(F;S)) (f : F -> cmd_) T
  ...
  ⊨ [pt,st] { \sum_i (dform '[ M i ] (P i)) } (cond_ M f) { Q }.
Proof.
move=>IH; have P1 i : ⊨l[pt,st] {(P i) T T} (f i) {Q W W}
  by rewrite -dirac_localP -!sqrd_linE.
move: (@R_IF_L pt st _ _ M _ _ _ _ _ P1); rewrite -dirac_localP -sqrd_linE; 
apply/R_back; rewrite linear_sum/=; apply eq_bigr=>i _.
by rewrite -!sdotd_correct /dform -sqrd_linE lin_adj.
Qed.
...
Lemma R_LP_P S (M : 'QM[H]_(bool;S)) b (D: cmd_) T 
  (P Q : 'D_T) :
  let R := dform '[M (~~b)] P + dform '[M b] Q in 
  P T T ⊑ \1 -> Q T T ⊑ \1 -> ⊨p { Q } D { R } -> ⊨p { R } (while_ M b D) { P }.
Proof.
move=>R PP PQ.
suff ->: R = '[ (M (~~b))^A \O P T T \O (M (~~b)) + (M b)^A \O Q T T \O (M b)].
rewrite !dirac_sqrP/= !dlin_id; apply: R_LP'_LP; [apply PP|apply PQ].
by rewrite /R /dform linearD/= -!sdotd_correct !lin_adj -!sqrd_linE.
Qed.
Global Arguments R_LP_P [S M b D T P Q].

(* move *)
Lemma led_elem (P Q : 'D[H]) : 
  P ⊑ Q -> forall S, P S S ⊑ Q S S.
Proof. by rewrite -subv_ge0=>/ged0P[+ _]S; move=>/(_ S); rewrite !diracE subv_ge0. Qed.

Lemma led_liftf S W (P : 'D[H]_S) (Q : 'D_W) :
...
rewrite sqrd_linE (sqrd_linE Q) !dlin_id !dlin_eq0r ?E// 1?eq_sym ?E//.
rewrite -liftf_lf_lef -[_ ⊑ Q W W]liftf_lf_lef !linear0; exact: le_trans.
Qed.

Lemma led_liftf_cplmt S W (P : 'D[H]_S) (Q : 'D_W) :
...
Proof.
rewrite sqrd_linE =>+[S' [W' [P3 [P4]]]] IH.
rewrite (sqrdE P3) (sqrdE P4) (sqrd_linE (SqrDirac P4)) dirac_localP; 
case: pt IH=>[IH/led_liftf/lef_trden| IH/led_liftf_cplmt/lef_trden];
rewrite !dlin_id=>P1 x.
apply/(le_trans (P1 x))/(le_trans (IH x)).
2: apply/(le_trans _ (P1 x))/(le_trans _ (IH x)).
...
Proof.
rewrite sqrd_linE =>+[S' [W' [P3 [P4]]]] IH.
rewrite (sqrdE P3) (sqrdE P4) (sqrd_linE (SqrDirac P3)) dirac_localP; 
case: pt IH=>[IH/led_liftf/lef_trden| IH/led_liftf_cplmt/lef_trden];
rewrite !dlin_id=>P1 x; [apply/(le_trans (IH x))|apply/(le_trans _ (IH x))];
...
Proof. by rewrite !dirac_sqrP/= !(dlin_id, diracE) alter_LPT. Qed.
...
Proof. by rewrite dirac_sqrP/= !(dlin_id, diracE); exact: Ax_N1N1_LT. Qed.

Lemma Ax_00_T st (c: cmd_) : ⊨t [st] { 0 } c { 0 }.
Proof. by move: (Ax_00_LT st set0 set0 c); rewrite -dirac_localP/= linear0. Qed.

Lemma R_Scale_T st (P Q : 'D) (c: cmd_) a :
  0 <= a -> ⊨t [st] { P } c { Q } -> 
    ⊨t [st] { a *: P } c { a *: Q }.
Proof.
move=>Pa[S [T]][PS [PT IH]].
rewrite (sqrdE PS) (sqrdE PT) dirac_sqrP/= !(dlin_id, diracE).
by apply: R_Scale_LT.
Qed.
...
Proof.
rewrite sqrd_linE (sqrd_linE Q1) (sqrd_linE P2) (sqrd_linE Q2)
!dirac_sqrP/= !(dlin_id, diracE); exact: R_Add_LT.
Qed.
...
Lemma R_Scale_P (P Q : 'D) (c: cmd_) a :
  0 <= a <= 1 -> ⊨p { P } c { Q } -> 
  ⊨p { a *: P } c { a *: Q }.
Proof.
move=>Pa[S [T]][PS [PT IH]].
...
by rewrite linear_sum/=; apply eq_bigr=>i _; rewrite linearZ/= -sqrd_linE.
Qed.
...
Lemma R_Inner S (u v : 'H_S) (c : cmd_) :
  ...
  ⊫t { (`|[< u ; v >]| ^+2)%:D } c { '[ [> v ; v <] ] }.
Proof.
by rewrite dnumI dnumZ1 dnumI !dirac_sqrP/= !(dlin_id,diracE); exact: R_Inner_LT.
Qed.
...
Proof.
by rewrite dnumI dnumZ1 dnumI !dirac_sqrP/= !(dlin_id,diracE); exact: R_PInner_LT.
Qed.
...
1,2: by rewrite -dirac_localP !lift_lfE !dlin_cast 
  -!tend_correct R_TI/= -?sqrd_linE// ?disjointXD.
rewrite -dirac_localP -!tend_correct !lift_lfE !dlin_cast -!tend_correct
  tendACA tendII tendACA tendII R_TI/= -?sqrd_linE//.
...
move: IH1=>[->//|Pi]; apply/orP; right. rewrite -!lin_gef0/= -!sqrd_linE Pi//=.
apply: tens_ge0_seq=>// i j Ri Rj nij; move: (dis i j Ri Rj nij); setdec.
by apply IH2.
Qed.
...
Lemma R_PC_seq_P (I : eqType) (r : seq I) (R : pred I) (S T : I -> {set L})
  ...
  (forall i, R i -> (0 : 'D[H]) ⊑ (Q i : 'D) ⊑ \1_(T i)) ->
...
Lemma RS_Frame pt st W W' (R : 'D_(W,W')) S S' (P : 'D_(S,S')) T T' (Q : 'D_(T,T')) c :
  (pt || no_while c) ->
  [disjoint (fvars c) & W'] -> 
...
apply: R_Frame=>//. 
by rewrite /= wfd_linE adjd_lin muld_correct dlin_id formV_ge0.
all: by rewrite adjdT tend_mul//; move: P3; rewrite disjointUX=>/andP[P5 P6].
Qed.
Global Arguments RS_Frame [pt st W W' R S S' P T T' Q c].

Lemma RS_Framel pt st W W' (R : 'D_(W,W')) S S' (P : 'D_(S,S')) T T' (Q : 'D_(T,T')) c :
  (pt || no_while c) ->
  [disjoint (fvars c) & W'] -> 
  [disjoint (S :|: T) & W] -> [disjoint (S' :|: T') & W'] ->
...
Lemma RV_Frame pt st W (R : 'Ket_W) S S' (P : 'D_(S,S')) T T' (Q : 'D_(T,T')) c :
  (pt || no_while c) ->
  [disjoint (fvars c) & W] -> 
  [disjoint (S' :|: T') & W] ->
  ⊨s [pt,st] { P } c { Q } -> ⊨s [pt,st] { P \⊗ R } c { Q \⊗ R }.
...
Lemma RV_Framel pt st W (R : 'Ket_W) S S' (P : 'D_(S,S')) T T' (Q : 'D_(T,T')) c :
  (pt || no_while c) ->
  [disjoint (fvars c) & W] -> 
  [disjoint (S' :|: T') & W] ->
  ⊨s [pt,st] { P } c { Q } -> ⊨s [pt,st] { R \⊗ P } c { R \⊗ Q }.
...
by rewrite ket_adj outerM dlin_id outp_le1// -hnorm_le1.
Qed.
...
Lemma Ax_In pt st S (v : 'NS_S) T (P: 'D_T) : 
  ⊨ [pt,st] { (dform '|v> P) } (init_ v) { P }.
Proof.
rewrite -(@R_El _ _ S)/= 1?tendC; last first.
apply: (R_back _ (Ax_In_onb [QONB of (onb2d eb_onbasis)] _)).
rewrite -(sumonb_outerM [QONB of (onb2d eb_onbasis)]) tend_suml.
apply eq_bigr=>i _; rewrite /= -[in RHS]dotd_mul/= -dotd_ten/=; last first.
rewrite -[in RHS]dotdA_cond/=; last first.
rewrite [_^A \· _]dotdC/=; last first.
rewrite !dformE/= adjdM adjdK/= -!dotd_mul/= !dotdA_cond//.
all: setdec.
Qed.
Global Arguments Ax_In [pt st S v T].

Lemma AxS_In pt st S (v : 'NS_S) T (P : 'D_T) : 
  ⊨s [pt,st] { '<v| \· P } (init_ v) { P }.
Proof.
rewrite stateE; apply: (R_back _ (Ax_In _)).
rewrite dformE/= adjdG ket_adj bra_adj -!dotd_mul/= !dotdA_cond//.
all: setdec.
Qed.
Global Arguments AxS_In [pt st S v T].

Lemma AxV_In pt st S (v : 'NS_S) T (P : 'Ket_T) : 
  ⊨s [pt,st] { '<v| \· P } (init_ v) { P }.
Proof.
rewrite stateE; apply: (R_back _ (Ax_In _)).
rewrite dformE/= adjdG ket_adj bra_adj -!dotd_mul/= !dotdA_cond//.
...
by rewrite/= dlin_id outp_le1// -hnorm_le1.
Qed.
...
rewrite !ket_adj !outerM !(adjd_num, dnumM).
...
(* initialization *)
Lemma tAx_In pt st T (x : vars T) (v : 'NS('Hs T)) (A : 'End[T]) S (P: 'D_S) : 
  [disjoint lb x & S] ->
  ⊨ [pt, st] { [< v%:V ; A v%:V >] *: (P : 'D) } (initt_ x v) { '[A,x] \⊗ P }.
Proof.
move=>dis; apply: (R_back _ (Ax_In _)).
by rewrite/= dformE ket_adj tlinT_bral//= dotdTrl//= tinnerG dnumTl -adj_dotEl.
Qed.
Global Arguments tAx_In [pt st T x v A S P].

Lemma tAx_InF pt st T (x : vars T) (v : 'NS('Hs T)) S (P: 'D_S) : 
  [disjoint lb x & S] ->
...
(* initialization *)
Lemma tAxV_In pt st T (x : vars T) (v : 'NS('Hs T)) (u : 'Hs T) S (P: 'Ket_S) : 
  [disjoint lb x & S] ->
  ⊨s [pt,st] { [< v%:V ; u >] *: (P : 'D) } (initt_ x v) { '|u,x> \⊗ P }.
Proof.
move=>dis. rewrite stateE adjdZ adjdT muldZr muldZl scalerA.
rewrite tend_mul ?disjointX0//= ket_adj touterM. 
apply: (R_back _ (tAx_In dis)).
by rewrite/= outpE dotpZr conj_dotp.
Qed.
Global Arguments tAxV_In [pt st T x v u S P].

Lemma tAxV_InF pt st T (x : vars T) (v : 'NS('Hs T)) S (P: 'Ket_S) : 
  [disjoint lb x & S] ->
...
Lemma tR_IF pt st (F : finType) T (x : vars T) (M : 'QM(F;'Hs T)) (f : F -> cmd_) S
  (P : F -> 'D_S) W (Q : 'D_W) : 
  (forall i, ⊨ [pt,st] { P i } (f i) { Q }) ->
  ⊨ [pt,st] { \sum_i (dform ('[ M i , x ]) (P i)) } (condt_ x M f) { Q }.
...
Lemma t2R_IF pt st (F : finType) (T1 T2 : ihbFinType) (x : vars T1) (y : vars T2)  
  (M : 'QM(F;'Hs (T1 * T2))) (f : F -> cmd_) S (P : F -> 'D_S) W (Q : 'D_W) :
  [disjoint lb x & lb y] ->
  (forall i, ⊨ [pt,st] { P i } (f i) { Q }) ->
  ⊨ [pt,st] { \sum_i (dform ('[ M i , x;y ]) (P i)) } (condt2_ x y M f) { Q }.
Proof. 
...
Lemma tR_LP_P S (P Q : 'D_S) T (x : vars T) (M : 'QM(bool;'Hs T)) b (D: cmd_) :
...
Lemma t2R_LP_P S (P Q : 'D_S) T1 T2 (x : vars T1) (y : vars T2) 
  (M : 'QM(bool;'Hs (T1 * T2))) b (D: cmd_) :
  let R := dform ('[ M (~~b) , x;y ]) P + dform ('[ M b , x;y ]) Q in 
  [disjoint lb x & lb y] ->
...
Lemma tbR_LP_P S (P Q : 'D_S) (x : vars bool) b (D: cmd_) :
...
Lemma Ax_UTP_seq pt st (F : eqType) (r : seq F) (P : pred F) (fd : F -> {set _}) 
  (Uf : forall i : F, 'FU[H]_(fd i)) W (Q : 'D_W) :
  uniq r -> (forall i j, P i -> P j -> (i != j) -> [disjoint fd i & fd j]) ->
  let U := \ten_(i <- r | P i) '[Uf i] in 
...
rewrite/= !dformE adjdT lin_adj [X in _ \· X]tendC -!dotd_ten/= ?dotdA//.
...
move: (@Ax_UTP_seq pt st _ _ _ _ Uf _ [SqrD of dform U^A Q] ur dis).
rewrite/= -/U -dformE dform_comp/= tens_adj dotd_mul/= tensM/==>[//|//|].
under eq_bigr do rewrite lin_adj muld_correct unitaryf_form.
rewrite tensI {1}dformE adjdI dotIdT/= dotdIT/= tendA tendII tendC.
...
Lemma AxV_UTP_seq pt st (F : eqType) (r : seq F) (P : pred F) (fd : F -> {set _}) 
  (Uf : forall i : F, 'FU[H]_(fd i)) W (Q : 'Ket_W) :
  uniq r -> (forall i j, P i -> P j -> (i != j) -> [disjoint fd i & fd j]) ->
  let U := \ten_(i <- r | P i) '[Uf i] in 
...
Lemma tAx_InP_seq pt st (F : eqType) (r : seq F) (P : pred F) 
  ...
  (v : forall i : F, 'NS('Hs (T i))) (A : forall i : F, 'End[T i]) W (Q : 'D_W):
  ...
  ⊨ [pt,st] { (\prod_(i <- r | P i) [< (v i)%:V ; A i (v i)%:V >]) *: (Q : 'D) } 
    ...
    { (\ten_(i <- r | P i) '[A i,fx i]) \⊗ Q }.
Proof.
move=>disx; elim: r W Q=>[W Q _ _ |i r IH W Q].
rewrite !big_nil bigd scale1r ten1d; exact: Ax_Sk.
...
Lemma tAx_InFP_seq pt st (F : eqType) (r : seq F) (P : pred F) 
  (T : F -> ihbFinType) (fx : forall i : F, vars (T i)) 
  (v : forall i : F, 'NS('Hs (T i))) W (Q : 'D_W):
  (forall i j, P i -> P j -> (i != j) -> [disjoint lb (fx i) & lb (fx j)]) ->
  uniq r ->
  [disjoint W & \bigcup_(i <- r | P i) lb (fx i)] ->
  ⊨ [pt,st] { Q } [for i <- r | P i do initt_ (fx i) (v i)]
    { (\ten_(i <- r | P i) '[[> v i ; v i <],fx i]) \⊗ Q }.
Proof.
...
Lemma tAxV_InP_seq pt st (F : eqType) (r : seq F) (P : pred F)
  (T : F -> ihbFinType) (fx : forall i : F, vars (T i)) 
  (v : forall i : F, 'NS('Hs (T i))) (u : forall i : F, 'Hs (T i)) W (Q : 'Ket_W):
  (forall i j, P i -> P j -> (i != j) -> [disjoint lb (fx i) & lb (fx j)]) ->
  uniq r ->
  [disjoint W & \bigcup_(i <- r | P i) lb (fx i)] ->
  ⊨s [pt,st] { (\prod_(i <- r | P i) [< (v i)%:V ; (u i) >]) *: (Q : 'D) } 
    [for i <- r | P i do initt_ (fx i) (v i)]
    { (\ten_(i <- r | P i) '|u i, fx i>) \⊗ Q }.
Proof.
...
Lemma tAxV_InFP_seq pt st (F : eqType) (r : seq F) (P : pred F) 
  (T : F -> ihbFinType) (fx : forall i : F, vars (T i)) 
  (v : forall i : F, 'NS('Hs (T i))) W (Q : 'Ket_W):
  (forall i j, P i -> P j -> (i != j) -> [disjoint lb (fx i) & lb (fx j)]) ->
  uniq r ->
  [disjoint W & \bigcup_(i <- r | P i) lb (fx i)] ->
  ⊨s [pt,st] { Q } [for i <- r | P i do initt_ (fx i) (v i)]
    { (\ten_(i <- r | P i) '|v i, fx i>) \⊗ Q }.
Proof.
