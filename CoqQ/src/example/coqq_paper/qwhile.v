...
Require Import mcextra mcaextra notation hermitian prodvect tensor mxpred extnum.
Require Import ctopology quantum inhabited.
...
Proof. by rewrite -idx_card; apply (bij_eq_card vt2i_bij). Qed.
...
Proof.
by move=>i; rewrite /idx_tv2v/idx_v2tv i2nK 
vt2iK enum_valK cast_ord_comp cast_ord_id.
Qed.
Lemma idx_v2tvK : cancel idx_v2tv idx_tv2v.
Proof.
by move=>i; rewrite /idx_tv2v/idx_v2tv 
cast_ord_comp cast_ord_id enum_rankK vi2tK n2iK.
Qed.
...
Definition pvars_tuple_t2i T n (x : 'I_n -> (vars T)) (t : n.-tuple T) :=
  packidx [ffun i : 'I_n => vt2i (x i) (t ~_ i)].
Lemma pvars_tuple_subproof T n (x : 'I_n -> (vars T)) 
  (dis : forall i j, i != j -> [disjoint (vset (x i)) & (vset (x j))]) :
  {i2t | cancel (@pvars_tuple_t2i T n x) i2t & 
    cancel i2t (@pvars_tuple_t2i T n x)}.
Proof.
exists (fun i=> [tuple k => vi2t (flatidx i k)])=>i.
by apply eq_from_tnth=>j; rewrite tnthE packidxK// ffunE vt2iK.
by apply/flatidx_inj/ffunP=>j; rewrite packidxK// ffunE tnthE vi2tK.
Qed.
Definition pvars_tuple T n x dis : vars (n.-tuple T) := 
  Var (@pvars_tuple_subproof T n x dis).

Lemma pvars_tuple_vset T n x dis :
  lb (@pvars_tuple T n x dis) = \bigcup_i (lb (x i)).
Proof. by []. Qed.
...
Definition tv2v (v : 'Hs T) : 'H[H]_(lb x) :=
  c2h (\col_i (h2c v) (idx_v2tv i) ord0).
Definition v2tv (v : 'H[H]_(lb x)) : 'Hs T := 
  c2h (\col_i (h2c v) (idx_tv2v x i) ord0).
Lemma tv2vK : cancel tv2v v2tv.
Proof. by move=>t; apply/h2c_inj/matrixP=>i j; rewrite !c2hK !mxE idx_tv2vK !ord1. Qed.
Lemma v2tvK : cancel v2tv tv2v.
Proof. by move=>t; apply/h2c_inj/matrixP=>i j; rewrite !c2hK !mxE idx_v2tvK !ord1. Qed.
...
Proof.
rewrite !dotp_mulmx !c2hK !mxE (reindex (@idx_tv2v _ _)).
...
Proof.
move=>a b c; apply/h2c_inj/matrixP=>i j.
by rewrite linearP/= !c2hK linearP/= !mxE.
Qed.
...
Proof. by apply/h2c_inj/matrixP=>i j; rewrite !c2hK !mxE. Qed.  

Definition tf2f (f : 'End[T]) : 'F[H]_(lb x) := 
  mx2h (\matrix_(i,j) (h2mx f) (idx_v2tv i) (idx_v2tv j)).
Definition f2tf (f : 'F[H]_(lb x)) : 'End[T] :=
  mx2h (\matrix_(i,j) (h2mx f) (idx_tv2v x i) (idx_tv2v x j)).
Lemma tf2fK : cancel tf2f f2tf.
Proof. by move=>t; apply/h2mx_inj/matrixP=>i j; rewrite !mx2hK !mxE !idx_tv2vK. Qed.
Lemma f2tfK : cancel f2tf tf2f.
Proof. by move=>t; apply/h2mx_inj/matrixP=>i j; rewrite !mx2hK !mxE !idx_v2tvK. Qed.
...
Proof. by move=>???; apply/h2mx_inj/matrixP=>i j; rewrite mx2hK !linearP/= !mx2hK !mxE. Qed.
...
Proof.
apply/h2mx_inj/matrixP=>i j; rewrite !(h2mx_comp, mxE, mx2hK)/= (reindex (@idx_tv2v _ _)).
...
Proof. by apply/h2mx_inj/matrixP=>i j; rewrite !mx2hK !mxE. Qed.
...
Proof.
apply/h2c_inj/matrixP=>i j; rewrite !applyfh !c2hK !mx2hK !mxE (reindex (@idx_tv2v _ _)).
by exists (@idx_v2tv _ x)=>k _; rewrite ?idx_tv2vK// idx_v2tvK.
by apply eq_bigr=>k _; rewrite !mxE idx_tv2vK.
Qed.
Lemma tf2f_trlf (f : 'End[T]) : \Tr (tf2f f) = \Tr f.
Proof.
rewrite/lftrace/=/mxtrace (reindex (@idx_tv2v _ _)).
by exists (@idx_v2tv _ x)=>k _; rewrite ?idx_tv2vK// idx_v2tvK.
by apply eq_bigr=>k _; rewrite mx2hK !mxE idx_tv2vK.
Qed.
...
Proof.
apply/h2c_inj/matrixP=>i j; rewrite !c2hK !mxE /idx_v2tv cast_ord_comp.
rewrite cast_ord_id enum_rankK deltav_unfold h2c_eb mxE ord1 eqxx andbT.
by rewrite (can2_eq (@vi2tK _ _ _ x) (@vt2iK _ _ _ x)) (can2_eq (@n2iK _ _ _) (@i2nK _ _ _)).
Qed.
...
Proof.
apply/cdvP=>i; rewrite cdvT !tv2v_delta !cdv_dv/=/pvars2_t2i.
...
Lemma pvars_tuple_t2tv (T : ihbFinType) (n : nat) (x : 'I_n -> (vars T))
  (dis : forall i j, i != j -> [disjoint lb (x i) & lb (x j)]) (t : n.-tuple T) :
  (tv2v (pvars_tuple dis) ''t) = 
  tenvm (fun i=>tv2v (x i) ''(t ~_ i)).
Proof.
pose tt := packidx [ffun i: 'I_n=> vt2i (x i) (t ~_ i) ].
transitivity (tenvm (fun i=>deltav (flatidx tt i))); last first.
by apply/tenvmP=>i; rewrite tv2v_delta/tt packidxK// ffunE.
by rewrite tv2v_delta/=/pvars_tuple_t2i -dv_splitm.
Qed.

Lemma pvars_tuple_tentv (T : ihbFinType) (n : nat) (x : 'I_n -> (vars T))
  (dis : forall i j, i != j -> [disjoint lb (x i) & lb (x j)]) (v : 'I_n -> 'Hs T) :
...
Lemma pvars_tuple_tentf (T : ihbFinType) (n : nat) (x : 'I_n -> (vars T))
  (dis : forall i j, i != j -> [disjoint lb (x i) & lb (x j)]) (v : 'I_n -> 'End[T]) :
...
Proof. by rewrite idx_card dim_setten big_set1 ihb_dim_cast. Qed.
...
Let vtuple := (fun i => vx i).
Lemma vtuple_build i j : i != j -> [disjoint lb (vtuple i) & lb (vtuple j)].
Proof. by rewrite /vset/= disjoint1X inE. Qed.
...
Lemma rcons_disjointx (T : ihbFinType) (x sl : vars T) n (sh : 'I_n -> (vars T)) : 
  (forall i : 'I_n.+1, [disjoint lb x & lb (frcons sh sl i)])
  <-> [disjoint lb x & lb sl] /\ (forall i, [disjoint lb x & lb (sh i)]).
Proof.
split=>[H1|[]P1 P2 i]. split=>[|i]. by move: (H1 ord_max); rewrite frcons_max.
by move: (H1 (nlift ord_max i)); rewrite frconsE.
by case: (unliftP ord_max i)=>[?->|->]; rewrite ?frcons_max ?frconsE.
Qed.

Lemma rcons_disjoint (T : ihbFinType) (sl : vars T) n (sh : 'I_n -> (vars T)) : 
  (forall i j, i != j -> [disjoint lb (frcons sh sl i) & 
      lb (frcons sh sl j)])
  <-> (forall i, [disjoint lb sl & lb (sh i)]) /\ 
      (forall i j, i != j -> [disjoint lb (sh i) & lb (sh j)]).
Proof.
split=>[H1|[]P1 P2 i j]. split=>[i|i j].
by move: (H1 _ _ (neq_lift ord_max i)); rewrite frcons_max frconsE.
by rewrite -(inj_eq (lift_inj (h := ord_max)))=>/H1; rewrite !frconsE.
case: (unliftP ord_max i)=>/=[?->|->]; case: (unliftP ord_max j)=>/=[?->|->].
by rewrite (inj_eq (lift_inj (h := ord_max))) !frconsE; apply: P2.
1,2: by move=>_; rewrite frconsE frcons_max// disjoint_sym.
by rewrite eqxx.
Qed.

Lemma cons_disjointx (T : ihbFinType) (x sh : vars T) n (sl : 'I_n -> (vars T)) : 
  (forall i : 'I_n.+1, [disjoint lb x & lb (fcons sh sl i)])
  <-> [disjoint lb x & lb sh] /\ (forall i, [disjoint lb x & lb (sl i)]).
Proof.
split=>[H1|[]P1 P2 i]. split=>[|i]. by move: (H1 ord0); rewrite fcons0.
by move: (H1 (nlift ord0 i)); rewrite fconsE.
by case: (unliftP ord0 i)=>[/=?->|->]; rewrite ?fcons0// fconsE.
Qed.

Lemma cons_disjoint (T : ihbFinType) (sh : vars T) n (sl : 'I_n -> (vars T)) : 
  (forall i j, i != j -> [disjoint lb (fcons sh sl i) & 
      lb (fcons sh sl j)])
  <-> (forall i, [disjoint lb sh & lb (sl i)]) /\ 
      (forall i j, i != j -> [disjoint lb (sl i) & lb (sl j)]).
Proof.
split=>[H1|[]P1 P2 i j]. split=>[i|i j].
by move: (H1 _ _ (neq_lift ord0 i)); rewrite fcons0 fconsE.
by rewrite -(inj_eq (@lift_inj _ ord0))=>/H1; rewrite !fconsE.
case: (unliftP ord0 i)=>/=[?->|->]; case: (unliftP ord0 j)=>/=[?->|->].
by rewrite (inj_eq (lift_inj (h := ord0))) !fconsE; apply: P2.
1,2: by move=>_; rewrite fconsE fcons0// disjoint_sym.
by rewrite eqxx.
Qed.
End DisjointTuple.
...
by rewrite adjfZ adj_outp conj_dotp adj_dotEr.
Qed.
...
by rewrite mul0r scale0r lef01. by rewrite mulfV ?pnatr_eq0// scale1r.
Qed.
...
Definition dirac_hoare (pt st : bool) (P Q : 'D[H]) (c: cmd) :=
  exists S T, (sqrd S P /\ sqrd T Q /\ local_hoare pt st (P S S) (Q T T) c).

Definition state_hoare (pt st : bool) (P Q : 'D[H]) (c: cmd) :=
...
Add Parametric Morphism L H pt st (P Q : 'D[H]) : 
  (@dirac_hoare L H pt st P Q)
  with signature (@eqcmd L H) ==> iff as eqcmd_dirac_hoare_mor.
Proof.
...
Add Parametric Morphism L H pt st (P Q : 'D[H]) : 
  ...
  with signature (@eqcmd L H) ==> iff as eqcmd_dirac_hoare_form_mor.
...
Lemma saturated_weak pt st (P: 'D[H]) (Q: 'D[H]) c :
  ⊨ [ pt , st ] { P } c { Q } -> ⊨ [ pt ] { P } c { Q }.
Proof. case: st=>//; exact: saturated_strong. Qed.
Lemma saturated_weakS pt st (P: 'D[H]) (Q: 'D[H]) c :
...
1,2: by rewrite linearBl/= comp_lfun1l !linearB/= opprB// addrNK.
...
Lemma relation_PT st c (P Q : 'D[H]) : 
...
Notation "''|' v , x >" := (dket (tv2v x v%VF)) : dirac_scope.
Notation "''|' v , x ; y >" := (dket (tv2v2 x y v%VF)) : dirac_scope.
Notation "''<' v , x |" := (dbra (tv2v x v%VF)) : dirac_scope.
Notation "''<' v , x ; y |" := (dbra (tv2v2 x y v%VF)) : dirac_scope.
Notation "''|' v >< u , x |" := ((dket (tv2v x v%VF)) \o (dbra (tv2v x u%VF))) : dirac_scope.
Notation "''|' v >< u , x ; y |" := (dket (tv2v2 x y v%VF) \o dbra (tv2v2 x y u%VF)) : dirac_scope.
Notation "''[' M , x ]" := (dlin (tf2f x M%VF)) : dirac_scope.
Notation "''[' M , x ; y ]" := (dlin (tf2f2 x y M%VF)) : dirac_scope.

(* fill the gap : dirac and tv2v tf2f tv2v2 tf2f2 *)
Section QExprExtra.
Local Open Scope dirac_scope.
Context (L : finType) (H : L -> chsType).
Notation vars T := (@tvars_of L H _ (Phant T)).
...
Lemma t1lin1 : \1_(lb x) = '[ \1, x ].
Proof. by rewrite tf2f1. Qed.
Lemma t2lin1 : \1_(lb x1 :|: lb x2) = '[ \1, x1;x2 ].
...
Lemma t1linM (M1 M2 : 'End[T]) : '[ M1, x ] \o '[ M2, x ] = '[ M1 \o M2, x ].
Proof. by rewrite linM tf2f_comp. Qed.
Lemma t2linM (M1 M2 : 'End[T1 * T2]) : '[ M1, x1;x2 ] \o '[ M2, x1;x2 ] = '[ M1 \o M2, x1;x2 ].
...
Lemma t1lin_ketM (M : 'End[T]) (v : 'Hs T) : '[ M, x ] \o '| v, x > = '| M v, x >.
Proof. by rewrite lin_ketM tf2f_apply. Qed.
Lemma t2lin_ketM (M : 'End[T1*T2]) (v : 'Hs (T1*T2)) : '[ M, x1;x2 ] \o '| v, x1;x2 > = '| M v, x1;x2 >.
...
Lemma t1lin_braM (M : 'End[T]) (v : 'Hs T) : '< v, x | \o '[ M, x ] = '< M^A v, x |.
Proof. by rewrite lin_braM tf2f_adj tf2f_apply. Qed.
Lemma t2lin_braM (M : 'End[T1*T2]) (v : 'Hs (T1*T2)) : '< v, x1;x2 | \o '[ M, x1;x2 ] = '< M^A v, x1;x2 |.
...
Lemma t1innerM (u v : 'Hs T) : '< u, x | \o '| v, x > = [<u ; v>]%:D.
Proof. by rewrite innerM tv2v_dot. Qed.
Lemma t2innerM (u v : 'Hs (T1 * T2)) : '< u, x1;x2 | \o '|v, x1;x2 > = [<u ; v>]%:D.
Proof. by rewrite innerM tv2v2_dot. Qed.
Lemma t1innerG (u v : 'Hs T) : '< u, x | \· '| v, x > = [<u ; v>]%:D.
Proof. by rewrite innerG tv2v_dot. Qed.
Lemma t2innerG (u v : 'Hs (T1 * T2)) : '< u, x1;x2 | \· '| v, x1;x2 > = [<u ; v>]%:D.
...
Lemma t1outerM (u v : 'Hs T) : '| v, x > \o '< u, x | = '[ [> v ; u <], x ].
Proof. by rewrite outerM tv2v_out. Qed.
Lemma t2outerM (u v : 'Hs (T1 * T2)) : '| v, x1;x2 > \o '< u, x1;x2 | = '[ [> v ; u <], x1;x2 ].
...
Proof. by rewrite -tlinT tendC linT tf2f2_ten. Qed.
Lemma tketTC (v1 : 'Hs T1) (v2 : 'Hs T2) : '| v1 ⊗t v2, x1;x2 > = '| v2 ⊗t v1, x2;x1 >.
Proof. by rewrite -tketT tendC ketT tv2v2_ten. Qed.
Lemma tbraTC (v1 : 'Hs T1) (v2 : 'Hs T2) : '< v1 ⊗t v2, x1;x2 | = '< v2 ⊗t v1, x2;x1 |.
Proof. by rewrite -tbraT tendC braT tv2v2_ten. Qed.
...
Proof. by rewrite -tf2f1 dotIdid// subsetUl. Qed.
Lemma tlin1Gr : '[ \1, x2 ] \· '[ M, x1;x2 ] = '[ M, x1;x2 ].
Proof. by rewrite -tf2f1 dotIdid// subsetUr. Qed.
Lemma tlinG1l : '[ M, x1;x2 ] \· '[ \1, x1 ] = '[ M, x1;x2 ].
Proof. by rewrite -tf2f1 dotdIid// subsetUl. Qed.
Lemma tlinG1r : '[ M, x1;x2 ] \· '[ \1, x2 ] = '[ M, x1;x2 ].
Proof. by rewrite -tf2f1 dotdIid// subsetUr. Qed.
...
by rewrite -[RHS]tlinG -[X in _ = X \· _]tlinT -tf2f1 
  -dotd_ten/= -?dotdA/= ?dotIdid// ?subsetUr// -disjoint_neqE.
Qed.
Lemma tlintGr : '[ N2, x2 ] \· '[ M, x1;x2 ] = '[(\1 ⊗f N2) \o M, x1;x2 ].
Proof.
by rewrite -[RHS]tlinG -[X in _ = X \· _]tlinT -tf2f1 tendC 
  -dotd_ten/= -?dotdA/= ?dotIdid// ?subsetUl// disjoint_sym.
Qed.
Lemma tlinGtl : '[ M, x1;x2 ] \· '[ N1, x1 ] = '[ M \o (N1 ⊗f \1), x1;x2 ].
Proof.
by rewrite -[RHS]tlinG -[X in _ = _ \· X]tlinT -tf2f1 tendC
  -dotd_ten/= ?dotdA/= ?dotdIid// ?subsetUr// disjoint_sym.
Qed.
Lemma tlinGtr : '[ M, x1;x2 ] \· '[ N2, x2 ] = '[ M \o (\1 ⊗f N2), x1;x2 ].
Proof.
by rewrite -[RHS]tlinG -[X in _ = _ \· X]tlinT -tf2f1
  -dotd_ten/= ?dotdA/= ?dotdIid// ?subsetUl.
Qed.
Lemma tketGl : '[ N1, x1 ] \· '| v, x1;x2 > = '|(N1 ⊗f \1) v, x1;x2 >.
Proof. 
by rewrite -[RHS]tlin_ketG -[X in _ = X \· _]tlinT -tf2f1 
  -dotd_ten/= -?dotdA/= ?dotIdid// ?subsetUr.
Qed.
Lemma tketGr : '[ N2, x2 ] \· '| v, x1;x2 > = '|(\1 ⊗f N2) v, x1;x2 >.
Proof.
by rewrite -[RHS]tlin_ketG -[X in _ = X \· _]tlinT -tf2f1 tendC 
  -dotd_ten/= -?dotdA/= ?dotIdid// ?subsetUl// disjoint_sym.
Qed.
Lemma tbraGl : '< v, x1;x2 | \· '[ N1, x1 ] = '<(N1^A ⊗f \1) v, x1;x2 |.
Proof.
by rewrite -adjf1 -tentf_adj -[RHS]tlin_braG -[X in _ = _ \· X]tlinT tendC -tf2f1
  -dotd_ten/= ?dotdA/= ?dotdIid// ?subsetUr// disjoint_sym.
Qed.
...
Variable (v : 'Hs T) (u : 'Hs (T1 * T2)) (S S' : {set L}) (e : 'D[H]_(S,S')).

Lemma t1lin_linTll : [disjoint lb x & S'] ->
  '[ M1, x ] \· ('[ M2, x ] \⊗ e) = '[ M1 \o M2, x ] \⊗ e.
Proof. by move=>P; rewrite lin_linTll// tf2f_comp. Qed.
Lemma t2lin_linTll : [disjoint lb x1 :|: lb x2 & S'] ->
  '[ N1, x1;x2 ] \· ('[ N2, x1;x2 ] \⊗ e) = '[ N1 \o N2, x1;x2 ] \⊗ e.
Proof. by move=>P; rewrite lin_linTll// tf2f2_comp. Qed.
Definition tlin_linTll := (t1lin_linTll, t2lin_linTll).

Lemma t1lin_linTlr : [disjoint lb x & S'] ->
  '[ M1, x ] \· (e \⊗ '[ M2, x ]) = e \⊗ '[ M1 \o M2, x ].
Proof. by move=>P; rewrite lin_linTlr// tf2f_comp. Qed.
Lemma t2lin_linTlr : [disjoint lb x1 :|: lb x2 & S'] ->
  '[ N1, x1;x2 ] \· (e \⊗ '[ N2, x1;x2 ]) = e \⊗ '[ N1 \o N2, x1;x2 ].
Proof. by move=>P; rewrite lin_linTlr// tf2f2_comp. Qed.
Definition tlin_linTlr := (t1lin_linTll, t2lin_linTll).

Lemma t1lin_linTrl : [disjoint lb x & S] ->
  ('[ M1, x ] \⊗ e) \· '[ M2, x ] = '[ M1 \o M2, x ] \⊗ e.
Proof. by move=>P; rewrite lin_linTrl// tf2f_comp. Qed.
Lemma t2lin_linTrl : [disjoint lb x1 :|: lb x2 & S] ->
  ('[ N1, x1;x2 ] \⊗ e) \· '[ N2, x1;x2 ] = '[ N1 \o N2, x1;x2 ] \⊗ e.
Proof. by move=>P; rewrite lin_linTrl// tf2f2_comp. Qed.
Definition tlin_linTrl := (t1lin_linTrl, t2lin_linTrl).

Lemma t1lin_linTrr : [disjoint lb x & S] ->
  (e \⊗ '[ M1, x ]) \· '[ M2, x ] = e \⊗ '[ M1 \o M2, x ].
Proof. by move=>P; rewrite lin_linTrr// tf2f_comp. Qed.
Lemma t2lin_linTrr : [disjoint lb x1 :|: lb x2 & S] ->
  (e \⊗ '[ N1, x1;x2 ]) \· '[ N2, x1;x2 ] = e \⊗ '[ N1 \o N2, x1;x2 ].
Proof. by move=>P; rewrite lin_linTrr// tf2f2_comp. Qed.
Definition tlin_linTrr := (t1lin_linTrr, t2lin_linTrr).
Definition tlin_linT := (tlin_linTll, tlin_linTlr, tlin_linTrl, tlin_linTrr).
...
Lemma t1linT_ketl : [disjoint lb x & S] ->
  ('[ M, x ] \⊗ e) \· '| v, x > = '| M v, x > \⊗ e.
Proof. by move=>P; rewrite dotdTrl//= tlin_ketG. Qed.
...
Lemma t1linT_ketr : [disjoint lb x & S] ->
  (e \⊗ '[ M, x ]) \· '| v, x > = e \⊗ '| M v, x >.
Proof. by move=>P; rewrite dotdTrr//= tlin_ketG. Qed.
...
Lemma t1linT_bral : [disjoint lb x & S'] ->
  '< v, x | \· ('[ M, x ] \⊗ e) = '< M^A v, x | \⊗ e.
Proof. by move=>P; rewrite dotdTll//= tlin_braG. Qed.
...
Lemma t1linT_brar : [disjoint lb x & S'] ->
  '< v, x | \· (e \⊗ '[ M, x ]) = e \⊗ '< M^A v, x |.
Proof. by move=>P; rewrite dotdTlr//= tlin_braG. Qed.
...
Lemma tketBT_tr : ( \ten_(i <- r | P i) '|F i, d i> )^T = \ten_(i <- r | P i) '<(F i)^*v, d i|.
Proof. by rewrite ketBT_tr; under eq_bigr do rewrite tv2v_conj. Qed.
Lemma tketBT_conj : (\ten_(i <- r | P i) '|F i, d i>)^C = \ten_(i <- r | P i) '|(F i)^*v, d i>.
Proof. by rewrite ketBT_conj; under eq_bigr do rewrite tv2v_conj. Qed.
Lemma tbraBT_tr : (\ten_(i <- r | P i) '<F i, d i|)^T = \ten_(i <- r | P i) '|(F i)^*v, d i>.
Proof. by rewrite braBT_tr; under eq_bigr do rewrite tv2v_conj. Qed.
Lemma tbraBT_conj : (\ten_(i <- r | P i) '<F i, d i|)^C = \ten_(i <- r | P i) '<(F i)^*v, d i|.
Proof. by rewrite braBT_conj; under eq_bigr do rewrite tv2v_conj. Qed.
Lemma touterMBT : (\ten_(i <- r | P i) '|F i, d i>) \o (\ten_(i <- r | P i) '<G i, d i|) 
  = \ten_(i <- r | P i) ('[[>F i; G i<], d i]).
Proof. by rewrite outerMBT; under eq_bigr do rewrite outerM tv2v_out. Qed.
Lemma touterGBT : (\ten_(i <- r | P i) '|F i, d i>) \· (\ten_(i <- r | P i) '<G i, d i|) 
  = \ten_(i <- r | P i) ('[[>F i; G i<], d i]).
Proof. by rewrite dotd_mul/= touterMBT. Qed.
End ketBT.

Lemma tinnerMBT (I : finType) (P : pred I) (d : I -> vars T) (F G : I -> 'Hs T) : 
  (forall i j : I, P i -> P j -> i != j -> [disjoint lb (d i) & lb (d j)]) ->
  (\ten_(i | P i) '<F i, d i|) \o (\ten_(i | P i) '|G i, d i>) 
...
Section pvars_tuple.
Variable (T : ihbFinType) (n : nat) (x : 'I_n -> (vars T)).
Variable (dis : forall i j, i != j -> [disjoint lb (x i) & lb (x j)]).

Lemma pvars_tupleEt (t : n.-tuple T) :
  '|''t, pvars_tuple dis> = \ten_i '|''(t ~_ i), x i>.
Proof. by rewrite pvars_t2tv tenvm_correct. Qed.

Lemma pvars_tupleEv (v : 'I_n -> 'Hs T) :
  '|tentv_tuple v, pvars_tuple dis> = \ten_i '| v i, x i>.
Proof. by rewrite pvars_tentv tenvm_correct. Qed.

Lemma pvars_tupleEf (v : 'I_n -> 'End[T]) :
  '[ tentf_tuple v , pvars_tuple dis ] = \ten_i '[ v i , x i ].
Proof. by rewrite pvars_tentf tenfm_correct. Qed.

Lemma pvarsEt_nseq (t : T) :
  '|''(nseq_tuple n t), pvars_tuple dis> = \ten_i '|''t, x i>.
Proof. by rewrite pvars_tupleEt; under eq_bigr do rewrite tnth_nseq. Qed.

Lemma pvarsEv_nseq (v : 'Hs T) :
  '|tentv_tuple (nseq_tuple n v), pvars_tuple dis> = \ten_i '| v, x i>.
Proof. by rewrite pvars_tupleEv; under eq_bigr do rewrite tuple_ffunE tnth_nseq. Qed.

Lemma pvarsEf_nseq (v : 'End[T]) :
  '[ tentf_tuple (nseq_tuple n v) , pvars_tuple dis ] = \ten_i '[ v , x i ].
Proof. by rewrite pvars_tupleEf; under eq_bigr do rewrite tuple_ffunE tnth_nseq. Qed.
End pvars_tuple.
...
Lemma pvars_dffunEt (t : {dffun forall i : F, TF i}) :
  '|''t, pvars_dffun dis> = \ten_i '|''(t i), x i>.
Proof. by rewrite pvars_t2tv tenvm_correct. Qed.

Lemma pvars_dffunEv (v : forall i : F, 'Hs (TF i)) :
  '|tentv_dffun v, pvars_dffun dis> = \ten_i '| v i, x i>.
Proof. by rewrite pvars_tentv tenvm_correct. Qed.

Lemma pvars_dffunEf (v : forall i : F, 'End[TF i]) :
  '[ tentf_dffun v , pvars_dffun dis ] = \ten_i '[ v i , x i ].
Proof. by rewrite pvars_tentf tenfm_correct. Qed.
