...
Fixpoint eval_S_scalar (s : S_scalar) : C :=
  ...
  | BK_inner T b k => [< (eval_B_bra b)^*v ; eval_K_ket k>]
  end
...
with eval_O_opt (T1 T2 : DType) (o : O_opt T1 T2) : 'Hom{T1, T2} :=
  ...
  | O_0 T1 T2 => 0%:VF
  | KB_outer T1 T2 k b => [> eval_K_ket k ; (eval_B_bra b)^*v <]
...
Proof. by rewrite/= tentv_conj -tentv_dot tentv_t2tv -surjective_pairing. Qed.
...
Proof. by rewrite/= -tentv_dot -tentv_conj tentv_t2tv -surjective_pairing. Qed.
...
by rewrite -tentf_apply tentv_t2tv -surjective_pairing tentf_tr.
Qed.
...
Lemma KET_CT_4 T (S : S_scalar) (B : B_bra T) :
  B_adj (B_scale S B) =k K_scale (S_conj S) (B_adj B).
