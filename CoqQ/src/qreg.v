...
Require Import mcextra mcaextra notation quantum inhabited autonat summable.
...
(* Reserved Notation "f x .. y" (in custom reg at level 0, only parsing,
                  ...
                  y constr at level 9). *)
Reserved Notation "''' x" (in custom reg at level 40, format "''' x", no associativity).
Reserved Notation "x '.1'" (in custom reg at level 50, format "x '.1'", left associativity).
Reserved Notation "x '.2'" (in custom reg at level 50, format "x '.2'", left associativity).
Reserved Notation "x '.[' i ]" (in custom reg at level 50, i constr, format "x .[ i ]", left associativity).
Reserved Notation "x '.-' i" (in custom reg at level 50, i constr, format "x '.-' i", left associativity).
Reserved Notation "( x , y , .. , z )" (in custom reg at level 0, format "( x ,  y ,  .. ,  z )", left associativity).
Reserved Notation "'dffun' x : aT => E" (in custom reg at level 0, x name, aT constr, format "'dffun'  x  :  aT  =>  E").
Reserved Notation "'dffun' x => E" (in custom reg at level 0, x name, format "'dffun'  x  =>  E").
Reserved Notation "'dffun' => E" (in custom reg at level 0, format "'dffun'  =>  E").
Reserved Notation "'dffun:' x" (in custom reg at level 0, x constr, format "'dffun:' x").
Reserved Notation "'ffun' x : aT => E" (in custom reg at level 0, x name, aT constr, format "'ffun'  x  :  aT  =>  E").
Reserved Notation "'ffun' x => E" (in custom reg at level 0, x name, format "'ffun'  x  =>  E").
Reserved Notation "'ffun' => E" (in custom reg at level 0, format "'ffun'  =>  E").
Reserved Notation "'ffun:' x" (in custom reg at level 0, x constr, format "'ffun:' x").
Reserved Notation "'tuple' x : aT => E" (in custom reg at level 0, x name, aT constr, format "'tuple'  x  :  aT  =>  E").
Reserved Notation "'tuple' x => E" (in custom reg at level 0, x name, format "'tuple'  x  =>  E").
Reserved Notation "'tuple' => E" (in custom reg at level 0, format "'tuple'  =>  E").
Reserved Notation "'tuple:' x" (in custom reg at level 0, x constr, format "'tuple:' x").
Reserved Notation "( 'dffun' x : aT => E )" (in custom reg at level 0, x name, aT constr). (* only parsing *)
Reserved Notation "( 'dffun' x => E )" (in custom reg at level 0, x name). (* only parsing *)
Reserved Notation "( 'dffun' => E )" (in custom reg at level 0). (* only parsing *)
Reserved Notation "( 'dffun:' x )" (in custom reg at level 0, x constr). (* only parsing *)
Reserved Notation "( 'ffun' x : aT => E )" (in custom reg at level 0, x name, aT constr). (* only parsing *)
Reserved Notation "( 'ffun' x => E )" (in custom reg at level 0, x name). (* only parsing *)
Reserved Notation "( 'ffun' => E )" (in custom reg at level 0). (* only parsing *)
Reserved Notation "( 'ffun:' x )" (in custom reg at level 0, x constr). (* only parsing *)
Reserved Notation "( 'tuple' x : aT => E )" (in custom reg at level 0, x name, aT constr). (* only parsing *)
Reserved Notation "( 'tuple' x => E )" (in custom reg at level 0, x name). (* only parsing *)
Reserved Notation "( 'tuple' => E )" (in custom reg at level 0). (* only parsing *)
Reserved Notation "( 'tuple:' x )" (in custom reg at level 0, x constr). (* only parsing *)
Reserved Notation "x : T" (in custom reg at level 99, T constr). (* only parsing *)
Reserved Notation "` x" (at level 10, format "` x").
...
(* the arg variable is regarded as qvar, when calling it, we can only *)
(* pass a quantum register with the same type that constructed from arg or local to it *)
...
Fixpoint qType_eq_op (t1 t2 : qType) :=
  ...
  | QPair t11 t12, QPair t21 t22 => (qType_eq_op t11 t21) && (qType_eq_op t12 t22)
  | QSum t11 t12, QSum t21 t22 => (qType_eq_op t11 t21) && (qType_eq_op t12 t22)
...
by f_equal; apply/funext=>i; move: P=>/(_ i)/forallP/(_ i); rewrite eqxx/==>/IH.
Qed.

Canonical qtype_eqMixin := EqMixin qType_eqP.
Canonical qtype_eqType := Eval hnf in EqType qType qtype_eqMixin.
...
Fixpoint qtype_to_tree (t : qType) : tree (nat + nat) :=
  ...
  | QDFFun n ft => Node 4 ((Leaf (inl n)) :: [seq qtype_to_tree (ft i) | i : 'I_n])
  end.
...
Definition qtype_choiceMixin := CanChoiceMixin qtype_to_treeK.
Canonical qtype_choiceType := Eval hnf in ChoiceType qType qtype_choiceMixin.
Definition qtype_countMixin := CanCountMixin qtype_to_treeK.
Canonical qtype_countType := Eval hnf in CountType qType qtype_countMixin.
...
Fixpoint cType_eq_op (t1 t2 : cType) :=
  ...
  | CPair t11 t12, CPair t21 t22 => (cType_eq_op t11 t21) && (cType_eq_op t12 t22)
  | CSum t11 t12, CSum t21 t22 => (cType_eq_op t11 t21) && (cType_eq_op t12 t22)
...
elim: t2=>//=[?->?->|?->?->|??->|?? IH]//; rewrite eqxx//=.
by apply/forallP=>i1; apply/forallP=>i2; apply/implyP; move=>/eqP/ord_inj->.
elim: t1 t2=>[?|||||? IH|? IH1 ? IH2|? IH1 ? IH2|? IH|?? IH|n fT IH]; case=>//=.
by move=>?/eqP->.
1,4: by move=>?/IH->.
1,2: by move=>??/andP[]/IH1->/IH2->.
by move=>??/andP[]/eqP->/IH->.
...
Canonical ctype_eqType := Eval hnf in EqType cType ctype_eqMixin.
...
Fixpoint ctype_to_tree (t : cType) : tree (qType + nat) :=
  ...
  | CDFFun n ft => Node 5 ((Leaf (inr n)) :: [seq ctype_to_tree (ft i) | i : 'I_n])
  end.
...
Definition ctype_choiceMixin := CanChoiceMixin ctype_to_treeK.
Canonical ctype_choiceType := Eval hnf in ChoiceType cType ctype_choiceMixin.
Definition ctype_countMixin := CanCountMixin ctype_to_treeK.
Canonical ctype_countType := Eval hnf in CountType cType ctype_countMixin.

Fixpoint eval_qtype (t : qType) : ihbFinType :=
  match t with
  | QUnit => unit_ihbFinType
  | QBool => bool_ihbFinType
  | QOrd n => ordinal_ihbFinType n
  | QOption T => option_ihbFinType (eval_qtype T)
  | QPair T1 T2 => prod_ihbFinType (eval_qtype T1) (eval_qtype T2)
  | QSum T1 T2 => sum_ihbFinType (eval_qtype T1) (eval_qtype T2)
  | QArray n T => tuple_ihbFinType n (eval_qtype T)
  | QDFFun n T => dffun_ihbFinType (fun i => eval_qtype (T i))
  end.

Fixpoint eval_ctype (t : cType) : ihbType :=
  match t with
  | QType t => FinInhabited.ihbType (eval_qtype t)
  | CNat => nat_ihbType
  | CInt => int_ihbType
  | CReal => real_ihbType hermitian.R
  | CCplx => complex_ihbType
  | COption T => option_ihbType (eval_ctype T)
  | CPair T1 T2 => prod_ihbType (eval_ctype T1) (eval_ctype T2)
  | CSum T1 T2 => sum_ihbType (eval_ctype T1) (eval_ctype T2)
  | CSeq T => seq_ihbType (eval_ctype T)
  | CArray n T => tuple_ihbType n (eval_ctype T)
  | CDFFun n T => dffun_ihbType (fun i => eval_ctype (T i))
  end.

Coercion QType : qType >-> cType.

Definition cast_qtype (t1 t2 : qType) (E : t1 = t2) (v : eval_qtype t1) : eval_qtype t2 :=
...
Lemma cast_qtype_comp (t1 t2 t3: qType) (E1 : t1 = t2) (E2 : t2 = t3) (v : eval_qtype t1) :
...
Definition cast_ctype (t1 t2 : cType) (E : t1 = t2) (v : eval_ctype t1) : eval_ctype t2 :=
  let: erefl in _ = t2 := E return eval_ctype t2 in v.

Lemma cast_ctype_inj {t1 t2 : cType} {E : t1 = t2} : injective (cast_ctype E).
Proof. by case: t2 / E. Qed.

Lemma cast_ctype_comp (t1 t2 t3: cType) (E1 : t1 = t2) (E2 : t2 = t3) (v : eval_ctype t1) :
...
Canonical string_eqMixin := EqMixin eqb_spec.
...
Definition string_choiceMixin := CanChoiceMixin string_to_seq_natK.
Canonical string_choiceType := Eval hnf in ChoiceType string string_choiceMixin.
Definition string_countMixin := CanCountMixin string_to_seq_natK.
Canonical string_countType := Eval hnf in CountType string string_countMixin.

Inductive cvar_r (Tc : cType) := CVar of string & string.
Definition cvname {Tc} (x : cvar_r Tc) := let: CVar pname name := x in (pname,name).

(* Definition cvar_of Tc (pT : phant Tc) := cvar_r Tc.
Notation cvar Tc := (cvar_of (Phant Tc)). *)

Definition cvtype {Tc} (v : cvar_r Tc) := Tc.

Inductive expr_ : Type -> Type :=
| var_  {Tc}   of cvar_r Tc : expr_ (eval_ctype Tc)
...
Definition cvar_eq (x y : cvar_r T) :=
  let: CVar x1 x2 := x in let: CVar y1 y2 := y in (x1 == y1) && (x2 == y2).

Lemma cvar_eqP (x y : cvar_r T) : reflect (x = y) (cvar_eq x y).
Proof.
by case: x y => [x1 x2] [y1 y2]; apply: (iffP andP) => [[]/eqP->/eqP->|[->->]].
Qed.

Definition cvar_eqMixin := EqMixin cvar_eqP.
Canonical cvar_eqType := EqType (cvar_r T) cvar_eqMixin.
End VarsEqType.

(* -------------------------------------------------------------------- *)
Lemma eq_cvar {t u : cType} (x : cvar_r t) (y : cvar_r u) :
      (Tagged cvar_r y = Tagged cvar_r x)
...
Canonical cmem_eqType := EqType cmem gen_eqMixin.
Canonical cmem_choiceType := ChoiceType cmem gen_choiceMixin.
...
Notation "''Ht' T" := (ihb_chsType (eval_qtype T%QT)) (at level 8, T at level 2, format "''Ht'  T").
Notation "''Hom{' T1 , T2 }" := ('Hom('Ht T1%QT, 'Ht T2%QT)) (at level 8, format "''Hom{' T1 ,  T2 }").
Notation "''End{' T }" := ('End('Ht T%QT)) (at level 8, format "''End{' T }").
...
Notation "x %:US" := (cst_ [unitary of x]) (at level 2, format "x %:US").
Notation "x %:DS" := (cst_ [den of x]) (at level 2, format "x %:DS").
Notation "x %:MS" := (cst_ [QM of x]) (at level 2, format "x %:MS").
Notation app2_ f x1 x2 := (app_ (app_ f x1) x2).
Notation "c %:S"    := (@cst_ _ c) (at level 2, format "c %:S").
Notation "c %:F e"    := (app_ (@cst_ _ c) e) (at level 2, format "c %:F  e").
Notation "c %:FF e1 e2"    := (app_ (app_ (@cst_ _ c) e1) e2) (at level 2, e1 at next level, format "c %:FF  e1 e2").
...
Notation "- e"  := (app_ (cst_ -%R) e)   : xsyn_scope.
Notation "e1 - e2"  := (app2_ (cst_ +%R) e1 (app_ (cst_ -%R) e2))   : xsyn_scope.
Notation "e1 * e2"  := (app2_ (cst_ *%R) e1 e2)   : xsyn_scope.
...
Notation "e1 '+n' e2"  := (app2_ (cst_ addn) e1 e2) (at level 50, e2 at next level, left associativity) : xsyn_scope.
Notation "e1 '-n' e2"  := (app2_ (cst_ subn) e1 e2) (at level 50, e2 at next level, left associativity) : xsyn_scope.
Notation "e1 '*n' e2"  := (app2_ (cst_ muln) e1 e2) (at level 40, e2 at next level, left associativity) : xsyn_scope.
...
Notation "'Option' T" := (QOption T) (at level 0, format "'Option' T"): qtype_scope.
Notation "T1 * T2" := (QPair T1 T2) : qtype_scope.
Notation "T1 + T2" := (QSum T1 T2) : qtype_scope.
Notation "T -[ n ]" := (QArray n T) (at level 0, n at next level, format "T -[ n ]"): qtype_scope.
Notation "{ 'qffun' 'forall' x < n , fT }" := (QDFFun (fun x : 'I_(n%N) => fT%QT)) (at level 0, x name) : qtype_scope.
Notation "{ 'qffun' 'forall' x , fT }" := (QDFFun (fun x => fT%QT)) (at level 0, x name) : qtype_scope.
...
Notation "'Option' T" := (COption T) (at level 0, format "'Option' T"): ctype_scope.
Notation "T1 * T2" := (CPair T1 T2) : ctype_scope.
Notation "T1 + T2" := (CSum T1 T2) : ctype_scope.
Notation "T -[ n ]" := (CArray n T) (at level 0, n at next level, format "T -[ n ]"): ctype_scope.
Notation "{ 'cffun' 'forall' x < n , fT }" := (CDFFun (fun x : 'I_(n%N) => fT%CT)) (at level 0, x name) : ctype_scope.
Notation "{ 'cffun' 'forall' x , fT }" := (CDFFun (fun x => fT%CT)) (at level 0, x name) : ctype_scope.
...
Definition qvar_eqMixin := EqMixin qvar_eqP.
Canonical qvar_eqType := EqType qvar qvar_eqMixin.
Lemma qvar_to_stringK : cancel 
  (fun x : qvar => let: QVar s1 s2 := x in (s1,s2))
    (fun s => match s with | (s1,s2) => QVar s1 s2 end).
Proof. by case. Qed.
Definition qvar_choiceMixin := CanChoiceMixin qvar_to_stringK.
Canonical qvar_choiceType := Eval hnf in ChoiceType qvar qvar_choiceMixin.
Definition qvar_countMixin := CanCountMixin qvar_to_stringK.
Canonical qvar_countType := Eval hnf in CountType qvar qvar_countMixin.
...
Coercion qname : qvart >-> qvar.

Inductive mkqvar (T : qType) (s1 s2 : string) := MKQVar & (G.[?(QVar s1 s2)]%fmap == Some T).
Definition to_qvart (T : qType) (s1 s2 : string) (q : mkqvar T s1 s2) :=
  let: MKQVar P := q in QVarT P.
Coercion to_qvart : mkqvar >-> qvart.
...
Qed.
Canonical qnat_eqMixin := EqMixin qnat_eqP.
Canonical qnat_eqType := Eval hnf in EqType qnat qnat_eqMixin.
...
Definition nat3_to_qnat (n : nat * nat * nat) : qnat :=
  ...
  | (0,0,0) => qnat_fst
  | (0,0,1) => qnat_snd
  | (1,n,i) => match (i < n)%N =P true with
               | ReflectT E => qnat_tuplei (Ordinal E)
               | _ => qnat_fst
               end
  | (2,n,i) => match (i < n)%N =P true with
               | ReflectT E => qnat_ffuni (Ordinal E)
               | _ => qnat_fst
               end
  | _ => qnat_fst
  end.
...
Proof. by case=>// n; case=>i /=; case: eqP=>// P1 P2; rewrite (eq_irrelevance P1 P2). Qed.

Definition qnat_choiceMixin := CanChoiceMixin qnat_to_nat3K.
Canonical qnat_choiceType := Eval hnf in ChoiceType qnat qnat_choiceMixin.
Definition qnat_countMixin := CanCountMixin qnat_to_nat3K.
Canonical qnat_countType := Eval hnf in CountType qnat qnat_countMixin.
...
Fixpoint qreg_index_to_tree (q : qreg_index) : tree _ :=
  ...
  | pair_index q1 q2 => Node 0 [:: qreg_index_to_tree q1; qreg_index_to_tree q2]
  | tuple_index n qt f => Node 1 ((Leaf (None,[::],n,[:: qt])) :: [seq (qreg_index_to_tree (f i)) | i : 'I_n])
  | ffun_index n qt f => Node 2 ((Leaf (None,[::],n,[seq qt i | i : 'I_n])) :: [seq (qreg_index_to_tree (f i)) | i : 'I_n])
  end.
...
Fixpoint tree_to_qreg_index t : qreg_index :=
  ...
  | Node 0 [:: t1 ; t2] => pair_index (tree_to_qreg_index t1) (tree_to_qreg_index t2)
...
by do ! f_equal; apply/funext=>i; rewrite ?(nth_map (Leaf (None,[::],0,[::])))/= 
  1?(nth_map ord0) ?(size_enum_ord,size_map)// nth_ord_enum.
Qed.

Definition qreg_index_eqMixin := CanEqMixin qreg_index_to_treeK.
Canonical qreg_index_eqType := EqType qreg_index qreg_index_eqMixin.
Definition qreg_index_choiceMixin := CanChoiceMixin qreg_index_to_treeK.
Canonical qreg_index_choiceType := Eval hnf in ChoiceType qreg_index qreg_index_choiceMixin.
Definition qreg_index_countMixin := CanCountMixin qreg_index_to_treeK.
Canonical qreg_index_countType := Eval hnf in CountType qreg_index qreg_index_countMixin.
...
Fixpoint qtype_of_index_rec (l : seq qnat) (t : option qType) :=
  ...
  | qnat_tuplei n i :: l, Some (QArray m T) => 
        match n =P m with
        | ReflectT P => qtype_of_index_rec l (Some T)
        | _ => None
        end
  | qnat_ffuni n i :: l, Some (QDFFun m T) => 
        match n =P m with
        | ReflectT P => qtype_of_index_rec l (Some (T (cast_ord P i)))
        | _ => None
        end
  | _, _ => None
  end.
...
Fixpoint qtype_of_index (s : qreg_index) : option qType :=
  ...
  | tuple_index n T fx => if [forall i, qtype_of_index (fx i) == Some T] then Some (QArray n T) else None
  | ffun_index n T fx => if [forall i, qtype_of_index (fx i) == Some (T i)] then
                      Some (QDFFun T) else None
  end.
...
Lemma qtype_of_index_rec_rcons (l : seq qnat) x (t : option qType) :
  ...
  | qnat_ffuni n i, Some (QDFFun m T) => 
        match n =P m with
        | ReflectT P => Some (T (cast_ord P i))
        | _ => None
        end
  | _, _ => None
  end.
...
all: case: t; last (by case b); case; (try by case: b);
  move=>m T; case: eqP=>?; by [case: b|apply: IH].
Qed.

Lemma qtype_of_index_rec_cat (T : option qType) s1 s2 :
  qtype_of_index_rec (s1 ++ s2) T = qtype_of_index_rec s2 (qtype_of_index_rec s1 T).
Proof.
...
- move: IHq=>/=; case: (index_of_qreg q)=>//; clear q.
  ...
  + by move=>/= n T q; case: [forall i, _].
- move: IHq=>/=; case: (index_of_qreg q)=>//; clear q.
  + by move=>x l; rewrite /= qtype_of_index_rec_rcons =>->; case: eqP.
  + by move=>/= q1 q2; case: (qtype_of_index q1)=>// T3; 
    case: (qtype_of_index q2)=>// T4 H; inversion H.
  + move=>m S q/=; case E: [forall _, _] =>// /esym P; 
    inversion P; rewrite/index_tuplei/=; case: eqP=>// eqn.
    by move: E=>/forallP/(_ (cast_ord eqn i))/eqP.
...
- move: IHq=>/=; case: (index_of_qreg q)=>//; clear q.
  ...
  + by move=>m S q/=; case: [forall _, _].
  + move=>m S q/=; case E: [forall _, _] =>// /esym P; 
    inversion P; rewrite/index_ffuni/=; case: eqP=>// eqn.
    case: m / H0 S q E P H1 eqn=>S q /forallP/(_ i)/eqP H1 _ /inj_existT-> ?.
...
Proof. by apply/negP=>P; move: P (index_typeK x)=>/eqP->/=; case: [forall _, _]. Qed.
...
Definition pair_to_basic_index (v : qvar * seq qnat) :=
  match is_basic_index v.1 v.2 =P true with
  | ReflectT E => Some (Basic_Index E)
  | _ => None
  end.

Lemma basic_indexK : pcancel basic_index_to_pair pair_to_basic_index.
Proof.
case=>qv sq P; rewrite /pair_to_basic_index /basic_index_to_pair.
by case: eqP=>//= P'; rewrite (eq_irrelevance P P').
Qed.

Lemma basic_indexKV : ocancel pair_to_basic_index basic_index_to_pair.
Proof. by case=>v s; rewrite /pair_to_basic_index /basic_index_to_pair; case: eqP. Qed.

Definition basic_index_eqMixin := PcanEqMixin basic_indexK.
Canonical basic_index_eqType := EqType basic_index basic_index_eqMixin.
Definition basic_index_choiceMixin := PcanChoiceMixin basic_indexK.
Canonical basic_index_choiceType := Eval hnf in ChoiceType basic_index basic_index_choiceMixin.
Definition basic_index_countMixin := PcanCountMixin basic_indexK.
Canonical basic_index_countType := Eval hnf in CountType basic_index basic_index_countMixin.

Fixpoint ssqnat (T : qType) (s : seq qnat) : seq (seq qnat) :=
  match T with
  | QUnit | QBool | QOrd _ | QOption _ | QSum _ _ => [:: s]
  | QPair T1 T2 => (ssqnat T1 (rcons s qnat_fst)) ++ (ssqnat T2 (rcons s qnat_snd))
  | QArray n T => flatten [seq (ssqnat T (rcons s (qnat_tuplei i))) | i : 'I_n]
  | QDFFun n fT => flatten [seq (ssqnat (fT i) (rcons s (qnat_ffuni i))) | i : 'I_n]
  end.

Definition enum_basic_index :=
  pmap pair_to_basic_index (flatten [seq (map 
    (fun j => ((fsval i), j)) (ssqnat (G.[fsvalP i]%fmap) [::])) | i : domf G]).

Lemma allpairs_uniq_dep1 [S T: eqType] [s : seq S] [t : S -> seq T] :
    uniq s ->
    {in s, forall x : S, uniq (t x)} ->
    {in s &, forall x y : S, forall i j, i \in t x -> j \in t y -> i = j -> x = y} ->
...
Proof.
elim: s=>[[x][]|a s IH [x][]]//=.
rewrite inE=>/orP[/eqP<- P|]; first by apply (subseq_trans P (prefix_subseq _ _)).
...
- move=>T2 T3 + /esym P; inversion P; case: T2 / H0 P => _ /(_ (T1 * T3)%QT erefl).
  by apply subseq_trans; rewrite -rcons_cat /ssqnat prefix_subseq.
- move=>T2 T3 + /esym P; inversion P; case: T3 / H0 P => _ /(_ (T2 * T1)%QT erefl).
  by apply subseq_trans; rewrite -rcons_cat /ssqnat suffix_subseq.
- move=>m; case: eqP=>// P; case: m / P => T2 + /esym PT; 
...
- move=>m; case: eqP=>// P; case: m / P; rewrite cast_ord_id => T2+ /esym PT.
...
apply/flatten_mapP; exists [` basic_index_in_domfG b]%fset; first by apply/mem_enum.
apply/mapP; exists (basic_index_to_pair b).2=>//; apply/basic_index_in_ssqnat.
by case: b=>/= v s P; rewrite/pair_to_basic_index; case: eqP=>//= P1; rewrite (eq_irrelevance P P1).
Qed.
...
Qed.
Definition basic_index_finMixin := Eval hnf in FinMixin basic_index_enumP.
Canonical basic_index_finType := Eval hnf in FinType basic_index basic_index_finMixin.
...
(* move *)
Lemma match_nat_eqP_depE n Tx (fx : 'I_n -> Tx) (cn : Tx) (i : 'I_n) :
  match n =P n with
  | ReflectT E => fx (cast_ord E i)
  | ReflectF _ => cn
  end = fx i.
Proof. by case: eqP=>// P; rewrite cast_ord_id. Qed.
...
- move=>v s; rewrite/index_tuplei/qi2seq/==>P1; rewrite P1.
  under [X in flatten X]eq_map do rewrite qtype_of_index_rec_rcons P1.
  by case: eqP=>// _; rewrite/= map_flatten pmap_flatten -2!map_comp.
- move=>m T' q/=; case: [forall _, _] =>// /esym P.
  inversion P; case: m / H0 q P=>q _; rewrite/qi2seq/=; f_equal.
  by under [RHS]eq_map do rewrite /index_tuplei/= match_nat_eqP_depE.
Qed.
...
- move=>v s; rewrite/index_ffuni/qi2seq/==>P1; rewrite P1.
  under [X in flatten X]eq_map do rewrite qtype_of_index_rec_rcons P1.
  case: eqP=>// ?. under [X in flatten X]eq_map do rewrite cast_ord_id.
  by rewrite/= map_flatten pmap_flatten -2!map_comp.
- move=>m T' q/=. case: [forall _, _] =>// /esym P.
  inversion P; case: m / H0 T' q P H1 =>T' q _ _; rewrite/qi2seq/=; f_equal.
  by under [RHS]eq_map do rewrite /index_ffuni/= match_nat_eqP_depE.
Qed.
...
have P_prime: forall T v s, qtype_of_index (prime_index v s) = Some T -> 
  ...
  - destruct (qtype_of_index_rec _) eqn:E=>//; case: q E=>// T1 T2 + /esym Pv.
    by inversion Pv; case: T1 / H0 Pv=> _ /IH[x Px]; exists (qreg_fst x)=>/=; rewrite Px.
  - destruct (qtype_of_index_rec _) eqn:E=>//; case: q E=>// T1 T2 + /esym Pv.
    by inversion Pv; case: T2 / H0 Pv=> _ /IH[x Px]; exists (qreg_snd x)=>/=; rewrite Px.
  - destruct (qtype_of_index_rec _) eqn:E=>//; case: q E=>// m T1; case: eqP=>// Pvm.
    case: m / Pvm=>/IH[x Px] /esym Pv; inversion Pv;
    by exists (qreg_tuplei i x); rewrite/= Px.
  - destruct (qtype_of_index_rec _) eqn:E=>//; case: q E=>// m T1; case: eqP=>// Pvm.
...
- move=>T1 IH1 T2 IH2; case=>//=[|x1 x2||]; ltest; first exact: P_prime.
  destruct (qtype_of_index x1) eqn:E1=>//; destruct (qtype_of_index x2) eqn:E2=>// Pv.
  by move: E1 E2; inversion Pv=>/IH1[x <-]/IH2[y <-]; exists (qreg_pair x y).
- move=>n T IH; case=>//=[||m T' x|]; ltest; first exact: P_prime.
  case E: [forall _, _] =>// /esym Pv; move: IH E; inversion Pv=>IH /forallP P1.
...
- move=>n T IH; case=>//=[|||m T' x]; ltest; first exact: P_prime.
  ...
  case: m / H0 T' x E Pv H1=>T' x + _ /inj_existT Pt; case: T' / Pt=>/forallP P1.
...
Proof. by rewrite/index_ffuni/=; case: eqP=>// P; rewrite cast_ord_id. Qed.

Lemma tuple_indexE n t (s : 'I_n -> qreg_index) (i : 'I_n) :
  index_tuplei (tuple_index t s) i = s i.
Proof. by rewrite/index_tuplei/=; case: eqP=>// P; rewrite cast_ord_id. Qed.
...
Proof.
case: x=>//; ltest.
- by move=>x s/= P i; rewrite qtype_of_index_rec_rcons P; case: eqP.
...
- move=>x s P1 P2; rewrite /qi2seq/= P1/= map_flatten pmap_flatten -2!map_comp.
...
Proof.
case: x=>//; ltest.
- by move=>x s/= P i; rewrite qtype_of_index_rec_rcons P; 
...
1-4,6: rewrite /qi2seq;
  case: x=>//=[v s IH|||]; ltest;
  rewrite/= IH size_pmap/= /pair_to_basic_index//; 
  by case: eqP=>//=; rewrite/is_basic_index/= IH.
...
Proof. by rewrite/eq_qreg/qr2seq/index_of_qreg/index_tuplei/=; case: eqP=>// p; rewrite cast_ord_id. Qed.
Lemma eq_qreg_ffuni n {T : 'I_n -> qType} i (x : forall i, qreg (T i)) :
  qreg_ffuni i (qreg_ffun x) =r x i.
Proof. by rewrite/eq_qreg/qr2seq/index_of_qreg/index_ffuni/=; case: eqP=>// p; rewrite cast_ord_id. Qed.

Definition eq_qregE := (eq_qreg_fst, eq_qreg_snd, eq_qreg_tuplei, eq_qreg_ffuni).
...
Proof. by move: (index_typeK x)=>/qi2seq_of_pair; rewrite /eq_qreg /qr2seq. Qed.

Lemma eq_qreg_tuple n T (x : qreg (T-[n])) :
  qreg_tuple (fun i => qreg_tuplei i x) =r x.
Proof. by move: (index_typeK x)=>/qi2seq_of_tuple; rewrite /eq_qreg /qr2seq. Qed.

Lemma eq_qreg_ffun n (T : 'I_n -> qType) (x : qreg {qffun T}) :
  qreg_ffun (fun i => qreg_ffuni i x) =r x.
Proof. by move: (index_typeK x)=>/qi2seq_of_ffun; rewrite /eq_qreg /qr2seq. Qed.
...
Definition disjoint_qreg_rec Tx Ty (x : qreg Tx) (y : qreg Ty) := [disjoint (qr2seq x) & (qr2seq y)].
Definition disjoint_qreg := disjoint_qreg_rec.
(* Definition disjoint_qregT (x y : qregT) := locked_with qreg_key ([disjoint (qr2seq x) & (qr2seq y)]). *)

(* define on the index *)
Definition disjoint_qregI_rec (x y : qreg_index) := [disjoint (qi2seq x) & (qi2seq y)].
...
Definition fset_qreg T (x : qreg T) := locked_with fset_qreg_key (seq_fset fset_qreg_key (qr2seq x)).
...
Proof. by rewrite [fset_qreg _](unlock [unlockable of fset_qreg x]); apply/seq_fsetE. Qed.
...
Lemma flatten_uniq_disjoint {T S: finType} (t : S -> seq T) : 
  uniq (flatten [seq t x | x : S]) = 
    [forall i, uniq (t i)] && [forall i, forall j, (i != j) ==> [disjoint t i & t j]].
Proof.
...
Inductive qreg_expr : qType -> Type :=
  ...
  (* to automatically determine the type, we only allow constant (meta variable) as indexes *)
  ...
  | qreg_tuple_expr {n T} (x : forall i : 'I_n, qreg_expr T):
    qreg_expr T-[n]
  | qreg_ffun_expr {n} {T : 'I_n -> qType} (x : forall i : 'I_n, qreg_expr (T i)):
    qreg_expr {qffun T}.
...
Coercion qreg_base : wf_qreg >-> qreg.
...
Coercion qreg_expr_base : wf_qreg_expr >-> qreg_expr.
...
Definition clone_wf_qreg_expr {T} e :=
  ...
  fun ewf (opL' := @WF_QReg_Expr T e ewf)
    & phant_id opL' opL => opL'.
...
Add Parametric Morphism {T1 T2} : (@qreg_fst T1 T2)
  with signature (@eq_qreg (QPair T1 T2)) ==> (@eq_qreg T1) as eq_qreg_qreg_fst.
Proof.
move=>x y; rewrite/eq_qreg !qr2seq_pairE.
move=>/(f_equal (take (qtype_size T1))); rewrite !take_size_cat ?size_qr2seq//.
Qed.

Add Parametric Morphism {T1 T2} : (@qreg_snd T1 T2)
  with signature (@eq_qreg (QPair T1 T2)) ==> (@eq_qreg T2) as eq_qreg_qreg_snd.
Proof.
move=>x y; rewrite/eq_qreg !qr2seq_pairE.
move=>/(f_equal (drop (qtype_size T1))); rewrite !drop_size_cat ?size_qr2seq//.
Qed.
...
case: (unliftP ord0 i)=>/=[j ->/(f_equal (drop (qtype_size T)))|->]; 
  last by move=>/(f_equal (take (qtype_size T))); rewrite !take_size_cat ?size_qr2seq.
...
1,3: by rewrite/qr2seq/qi2seq/=/index_tuplei/=; case: eqP=>// ?; rewrite cast_ord_id.
...
case: (unliftP ord0 i)=>/=[j ->/(f_equal (drop (qtype_size (T ord0))))|->];
  last by move=>/(f_equal (take (qtype_size (T ord0)))); rewrite !take_size_cat ?size_qr2seq.
rewrite !drop_size_cat ?size_qr2seq// -map_comp - [in X in _ = X]map_comp => P1.
...
1,3: by rewrite/qr2seq/qi2seq/=/index_ffuni/=; case: eqP=>// ?; rewrite cast_ord_id.
...
Add Parametric Morphism {T1 T2} : (@disjoint_qreg T1 T2)
  with signature (@eq_qreg T1) ==> (@eq_qreg T2) ==> (@eq bool) as eq_qreg_disjoint.
...
Notation "''' x" := (qreg_prime x) (in custom reg at level 40, format "''' x", no associativity).
Notation "x '.1'" := (qreg_fst x) (in custom reg at level 50, format "x '.1'", left associativity).
Notation "x '.2'" := (qreg_snd x) (in custom reg at level 50, format "x '.2'", left associativity).
Notation "x '.[' i ]" := (qreg_tuplei i x) (in custom reg at level 50, i constr, format "x .[ i ]", left associativity).
Notation "x '.-[' i ]" := (qreg_ffuni i x) (in custom reg at level 50, i constr, format "x '.-[' i ]", left associativity).
Notation "( x , y , .. , z )" := (qreg_pair .. (qreg_pair x y) .. z) (in custom reg at level 0, format "( x ,  y ,  .. ,  z )", left associativity).
Notation "'ffun' x : aT => E" := (qreg_ffun (fun x : (aT)%FORM => E)) (in custom reg at level 0, x name, aT constr, format "'ffun'  x  :  aT  =>  E").
Notation "'ffun' x => E" := (qreg_ffun (fun x => E)) (in custom reg at level 0, x name, format "'ffun'  x  =>  E").
Notation "'ffun' => E" := (qreg_ffun (fun _ => E)) (in custom reg at level 0, format "'ffun'  =>  E").
Notation "'ffun:' x" := (qreg_ffun x) (in custom reg at level 0, x constr, only parsing).
Notation "'tuple' x : aT => E" := (qreg_tuple (fun x : (aT)%FORM => E)) (in custom reg at level 0, x name, aT constr, format "'tuple'  x  :  aT  =>  E").
Notation "'tuple' x => E" := (qreg_tuple (fun x => E)) (in custom reg at level 0, x name, format "'tuple'  x  =>  E").
Notation "'tuple' => E" := (qreg_tuple (fun _ => E)) (in custom reg at level 0, format "'tuple'  =>  E").
Notation "'tuple:' x" := (qreg_tuple x) (in custom reg at level 0, x constr, only parsing).
Notation "( 'ffun' x : aT => E )" := (qreg_ffun (fun x : (aT)%FORM => E)) (in custom reg at level 0, x name, aT constr, only parsing).
Notation "( 'ffun' x => E )" := (qreg_ffun (fun x => E)) (in custom reg at level 0, x name, only parsing).
Notation "( 'ffun' => E )" := (qreg_ffun (fun _ => E)) (in custom reg at level 0, only parsing).
Notation "( 'ffun:' x )" := (qreg_ffun x) (in custom reg at level 0, x constr, only parsing).
Notation "( 'tuple' x : aT => E )" := (qreg_tuple (fun x : (aT)%FORM => E)) (in custom reg at level 0, x name, aT constr, only parsing).
Notation "( 'tuple' x => E )" := (qreg_tuple (fun x => E)) (in custom reg at level 0, x name, only parsing).
Notation "( 'tuple' => E )" := (qreg_tuple (fun _ => E)) (in custom reg at level 0, only parsing).
Notation "( 'tuple:' x )" := (qreg_tuple x) (in custom reg at level 0, x constr, only parsing).
Notation "x : T" := (x : qreg (T)%QT) (in custom reg at level 99, T constr, only parsing).

Notation "<{{ e }}>" := e (at level 0, e custom reg_expr at level 99) : reg_scope.
Notation "( x )" := x (in custom reg_expr at level 0) : reg_scope.
Notation "x" := x (in custom reg_expr at level 0, x constr at level 0) : reg_scope.
Notation "f x .. y" := (.. (f x) .. y)
                  (in custom reg_expr at level 0, only parsing,
                  f constr at level 0, x constr at level 9,
                  y constr at level 9) : reg_scope.
Notation "''' x" := (qreg_cst x) (in custom reg_expr at level 40, x custom reg, format "''' x", no associativity).
Notation "x '.1'" := (qreg_fst_expr x) (in custom reg_expr at level 50, format "x '.1'", left associativity).
Notation "x '.2'" := (qreg_snd_expr x) (in custom reg_expr at level 50, format "x '.2'", left associativity).
Notation "x '.[' i ]" := (qreg_tuplei_expr (i)%X x) (in custom reg_expr at level 50, i constr, format "x .[ i ]", left associativity).
Notation "x '.-[' i ]" := (qreg_ffuni_expr (i)%X x) (in custom reg_expr at level 50, i constr, format "x '.-[' i ]", left associativity).
Notation "( x , y , .. , z )" := (qreg_pair_expr .. (qreg_pair_expr x y) .. z) (in custom reg_expr at level 0, format "( x ,  y ,  .. ,  z )", left associativity).
Notation "'ffun' x : aT => E" := (qreg_ffun_expr (fun x : (aT)%FORM => E)) (in custom reg_expr at level 0, x name, aT constr, format "'ffun'  x  :  aT  =>  E").
Notation "'ffun' x => E" := (qreg_ffun_expr (fun x => E)) (in custom reg_expr at level 0, x name, format "'ffun'  x  =>  E").
Notation "'ffun' => E" := (qreg_ffun_expr (fun _ => E)) (in custom reg_expr at level 0, format "'ffun'  =>  E").
Notation "'ffun:' x" := (qreg_ffun_expr x) (in custom reg_expr at level 0, x constr, only parsing).
Notation "'tuple' x : aT => E" := (qreg_tuple_expr (fun x : (aT)%FORM => E)) (in custom reg_expr at level 0, x name, aT constr, format "'tuple'  x  :  aT  =>  E").
Notation "'tuple' x => E" := (qreg_tuple_expr (fun x => E)) (in custom reg_expr at level 0, x name, format "'tuple'  x  =>  E").
Notation "'tuple' => E" := (qreg_tuple_expr (fun _ => E)) (in custom reg_expr at level 0, format "'tuple'  =>  E").
Notation "'tuple:' x" := (qreg_tuple_expr x) (in custom reg_expr at level 0, x constr, only parsing).
Notation "( 'ffun' x : aT => E )" := (qreg_ffun_expr (fun x : (aT)%FORM => E)) (in custom reg_expr at level 0, x name, aT constr, only parsing).
Notation "( 'ffun' x => E )" := (qreg_ffun_expr (fun x => E)) (in custom reg_expr at level 0, x name, only parsing).
Notation "( 'ffun' => E )" := (qreg_ffun_expr (fun _ => E)) (in custom reg_expr at level 0, only parsing).
Notation "( 'ffun:' x )" := (qreg_ffun_expr x) (in custom reg_expr at level 0, x constr, only parsing).
Notation "( 'tuple' x : aT => E )" := (qreg_tuple_expr (fun x : (aT)%FORM => E)) (in custom reg_expr at level 0, x name, aT constr, only parsing).
Notation "( 'tuple' x => E )" := (qreg_tuple_expr (fun x => E)) (in custom reg_expr at level 0, x name, only parsing).
Notation "( 'tuple' => E )" := (qreg_tuple_expr (fun _ => E)) (in custom reg_expr at level 0, only parsing).
Notation "( 'tuple:' x )" := (qreg_tuple_expr x) (in custom reg_expr at level 0, x constr, only parsing).
Notation "x : T" := (x : qreg_expr (T)%QT) (in custom reg_expr at level 99, T constr, only parsing).
...
Ltac luf := repeat rewrite/eval_index/index_fst/index_snd/index_tuplei/index_ffuni
  /disjoint_qregI/disjoint_qregI_rec/valid_qregI/valid_qregI_rec
  /disjoint_qreg/disjoint_qreg_rec/valid_qreg/valid_qreg_rec
  /qr2seq/qi2seq/=.
...
Ltac lnil := try (intros; match goal with 
  ...
  | [|- is_true [disjoint _ & [::]]] => rewrite disjoint_sym; apply disjointNilX 
  end).
...
- case: l1=>//=; luf.
  ...
  - move=>v s; rewrite /qi2seq/= qtype_of_index_rec_rcons.
    case E1: (qtype_of_index_rec _) =>[T|//]; case: T E1=>//=; lnil.
    by intros; move: H; rewrite map_cat pmap_cat disjoint_cat=>/andP[].
  - move=>n i v s; rewrite /qi2seq/= qtype_of_index_rec_rcons.
    case E1: (qtype_of_index_rec _) =>[T|//]; case: T E1=>//=; lnil.
    intros; case: eqP=>[Pv| _]; lnil; case: _ / Pv E1 H=> P1.
    by rewrite map_flatten pmap_flatten -2!map_comp disjoint_flatten=>/forallP/(_ i).
  - move=>n i v s; rewrite /qi2seq/= qtype_of_index_rec_rcons.
    case E1: (qtype_of_index_rec _) =>[T|//]; case: T E1=>//=; lnil.
    intros; case: eqP=>[Pv| _]; lnil; case: _ / Pv fT E1 H=> fT P1; rewrite cast_ord_id.
    by rewrite map_flatten pmap_flatten -2!map_comp disjoint_flatten=>/forallP/(_ i).
...
- move=>n T q; rewrite /qi2seq/=; case: l1=>//=[||???|m i IH]; luf; lnil.
  case: eqP=>//=[Pv|?]; lnil; case: n / Pv T q IH=>T q; 
...
Lemma eval_index_rec_cat v s1 s2 : eval_index_rec (prime_index v s1) s2 = prime_index v (s1 ++ s2).
Proof.
...
- case: x=>//=[a s|???].
  - rewrite qtype_of_index_rec_rcons; case E1: (qtype_of_index_rec _ _)=>[T|]//.
    ...
    by rewrite map_flatten pmap_flatten -2!map_comp flatten_uniq_disjoint=>/andP[]/forallP/(_ i).
  - by case: eqP=>// P; rewrite flatten_uniq_disjoint=>/andP[]/forallP/(_ (cast_ord P i)).
- case: x=>//=[a s|???].
  - rewrite qtype_of_index_rec_rcons; case E1: (qtype_of_index_rec _ _)=>[T|]//.
    case: T E1=>//=m T _; case: eqP=>// Pv; case: m / Pv T => T.
    by rewrite cast_ord_id map_flatten pmap_flatten -2!map_comp flatten_uniq_disjoint=>/andP[]/forallP/(_ i).
  - by case: eqP=>// P; rewrite flatten_uniq_disjoint=>/andP[]/forallP/(_ (cast_ord P i)).
Qed.
...
-  move=>v s; luf; case: q1=>[||n i|n i];
  ...
  - move=>T1 T2 PT; case: q2=>[||m j|m j]; 
    ...
    move: (@eval_index_disjoint (prime_index v (rcons s qnat_fst)) (prime_index v (rcons s qnat_snd)) s1 s2).
    by luf; rewrite !eval_index_rec_cat !cat_rcons/= !qtype_of_index_rec_cat PT/= !qtype_of_index_rec_rcons PT=>/(_ P).
  - move=>T1 T2 PT; case: q2=>[||m j|m j]; 
    rewrite eval_index_rec_cat cat_rcons/= qtype_of_index_rec_cat PT/=; lnil.
      rewrite map_cat pmap_cat cat_uniq_disjoint=>/and3P[] _ P _ _.
      move: (@eval_index_disjoint (prime_index v (rcons s qnat_snd)) (prime_index v (rcons s qnat_fst)) s1 s2).
      by luf; rewrite disjoint_sym !eval_index_rec_cat !cat_rcons/= !qtype_of_index_rec_cat PT/= !qtype_of_index_rec_rcons PT=>/(_ P).
    rewrite map_cat pmap_cat cat_uniq_disjoint=>/and3P[]P1 _ P2 P3.
    rewrite -!cat_rcons.
    move: (valid_qregIP_prime v (rcons s qnat_snd))=>/IH/(_ P3); luf; 
    by rewrite !eval_index_rec_cat !cat_rcons/= !qtype_of_index_rec_cat PT/=.
  - move=>m T; case: eqP; lnil=>Pv; case: m / Pv => PT.
    case: q2=>[||m j|m j]; rewrite eval_index_rec_cat cat_rcons/= qtype_of_index_rec_cat PT/=; lnil.
    case: eqP; lnil=>/esym Pv; case: m / Pv j=>j; case: eqP=>[/ord_inj ->/=|/eqP/= nij + _].
      rewrite map_flatten pmap_flatten -2!map_comp flatten_uniq_disjoint=>/andP[]/forallP/(_ j) Pu _ P1.
      move: (valid_qregIP_prime v (rcons s (qnat_tuplei j)))=>/IH/(_ P1); luf; 
      by rewrite !eval_index_rec_cat !cat_rcons/= !qtype_of_index_rec_cat PT/=; case: eqP.
    rewrite map_flatten pmap_flatten -2!map_comp flatten_uniq_disjoint=>/andP[] _ /forallP/(_ i)/forallP/(_ j)/implyP/(_ nij)/= P.
    move: (@eval_index_disjoint (prime_index v (rcons s (qnat_tuplei i))) (prime_index v (rcons s (qnat_tuplei j))) s1 s2).
    luf; rewrite !eval_index_rec_cat !cat_rcons/= !qtype_of_index_rec_cat PT/= !qtype_of_index_rec_rcons PT.
    by case: eqP=>// _ /(_ P).
  - move=>m T; case: eqP; lnil=>Pv; case: m / Pv T => T PT.
    case: q2=>[||m j|m j]; rewrite eval_index_rec_cat cat_rcons/= qtype_of_index_rec_cat PT/=; lnil.
    case: eqP; lnil=>Pv; move: Pv {+}Pv=>/esym Pv; case: m / Pv j=>j ?; rewrite cast_ord_id.
    case: eqP=>[/ord_inj ->/=|/eqP/= nij + _].
      rewrite map_flatten pmap_flatten -2!map_comp flatten_uniq_disjoint=>/andP[]/forallP/(_ j) Pu _ P1.
      move: (valid_qregIP_prime v (rcons s (qnat_ffuni j)))=>/IH/(_ P1); luf; 
      rewrite !eval_index_rec_cat !cat_rcons/= !qtype_of_index_rec_cat PT/=;
      by case: eqP=>// ?; rewrite !cast_ord_id.
    rewrite map_flatten pmap_flatten -2!map_comp flatten_uniq_disjoint=>/andP[] _ /forallP/(_ i)/forallP/(_ j)/implyP/(_ nij)/= P.
    move: (@eval_index_disjoint (prime_index v (rcons s (qnat_ffuni i))) (prime_index v (rcons s (qnat_ffuni j))) s1 s2).
    luf; rewrite !eval_index_rec_cat !cat_rcons/= !qtype_of_index_rec_cat PT/= !qtype_of_index_rec_rcons PT.
    by case: eqP=>// ?; rewrite !cast_ord_id=>/(_ P).
- move=>i1 i2 Pi. case: q1; case: q2; luf.
  3,4,7-16: rewrite ?eval_index_recF/= 1?[[disjoint _ & [::]]]disjoint_sym; intros; apply disjointNilX.
  1,4: apply: IH. 3,4: move=> _; apply/eval_index_disjoint; rewrite ?[_ _ i1]disjoint_qregIC.
  1-4: by move: Pi; luf; rewrite cat_uniq_disjoint=>/and3P[].
- move=>n T q Hi; case: q1=>[||m i|m i]; luf.
    3: case: eqP. 1-2,4-5: rewrite eval_index_recF/=; intros; apply disjointNilX.
    move=>P; move: P {+}P=>/esym P; case: m / P i=>i P; rewrite cast_ord_id; clear P.
    case: q2=>[||m j|m j]; luf.
    3: case: eqP. 1-2,4-5: rewrite eval_index_recF/= disjoint_sym; intros; apply disjointNilX.
  move=>P; move: P {+}P=>/esym P; case: m / P j=>j P; rewrite cast_ord_id; clear P.
  case: eqP=>[/ord_inj ->/=|/eqP];[apply/IH|move=>Pi _; apply/eval_index_disjoint];
  move: Hi; luf; rewrite flatten_uniq_disjoint=>/andP[]/forallP/(_ j)//.
  by move=> _ /forallP/(_ i)/forallP/(_ j)/implyP/(_ Pi).
- move=>n T q Hi; case: q1=>[||m i|m i]; luf.
    4: case: eqP. 1-3,5: rewrite eval_index_recF/=; intros; apply disjointNilX.
    move=>P; move: P {+}P=>/esym P; case: m / P i=>i P; rewrite cast_ord_id; clear P.
    case: q2=>[||m j|m j]; luf.
    4: case: eqP. 1-3,5: rewrite eval_index_recF/= disjoint_sym; intros; apply disjointNilX.
  move=>P; move: P {+}P=>/esym P; case: m / P j=>j P; rewrite cast_ord_id; clear P.
  case: eqP=>[/ord_inj ->/=|/eqP];[apply/IH|move=>Pi _; apply/eval_index_disjoint];
  move: Hi; luf; rewrite flatten_uniq_disjoint=>/andP[]/forallP/(_ j)//.
  by move=> _ /forallP/(_ i)/forallP/(_ j)/implyP/(_ Pi).
...
Lemma disjoint_qregIP_prime2 (x1 x2 : qvar) s11 s12 s21 s22 : 
    x1 != x2 ->
      disjoint_qregI (eval_index (prime_index x1 s11) s12) (eval_index (prime_index x2 s21) s22).
Proof.
...
Proof. by move=>P; rewrite disjoint_qregIC; apply/disjoint_qregI_tupleL=>i; rewrite disjoint_qregIC. Qed.
...
Proof. by move=>P; rewrite disjoint_qregIC; apply/disjoint_qregI_ffunL=>i; rewrite disjoint_qregIC. Qed.
...
Proof. by move=>P; rewrite disjoint_qregC; apply/disjoint_qreg_tupleL=>i; rewrite disjoint_qregC. Qed.

Lemma disjoint_qreg_ffunL n (Tx : 'I_n -> qType) (x : forall i, qreg (Tx i)) Ty (y : qreg Ty) :
  (forall i : 'I_n, disjoint_qreg (x i) y) ->
    disjoint_qreg (qreg_ffun x) y.
Proof. by luf=>/forallP; rewrite disjoint_flatten. Qed.

Lemma disjoint_qreg_ffunR Ty (y : qreg Ty) n (Tx : 'I_n -> qType) (x : forall i, qreg (Tx i)) :
  (forall i : 'I_n, disjoint_qreg y (x i)) -> 
      disjoint_qreg y (qreg_ffun x).
Proof. by move=>P; rewrite disjoint_qregC; apply/disjoint_qreg_ffunL=>i; rewrite disjoint_qregC. Qed.

End Theory.

(* tactic for solving qvar_diff from hypothesis qvar_dis                     *)
Ltac tac_qvar_diff := repeat match goal with
    | [ |- is_true (negb (@eq_op qvar_eqType _ _ ))] => (* idtac "trivial"; *)
...
Ltac tac_valid_qreg := match goal with
  ...
  | [ |- is_true (valid_qregI (index_of_qreg (qreg_base _))) ] => by apply/qreg_is_valid
  | [ |- is_true (valid_qreg (qsem_of (qreg_expr_base _) _)) ] => by apply/qreg_expr_is_valid
  | [ |- is_true (valid_qregI (index_of_qreg (qsem_of (qreg_expr_base _) _))) ] => by apply/qreg_expr_is_valid
...
Ltac tac_disjoint_qreg := match goal with
  | [ |- is_true (disjoint_qreg (qreg_prime _) (qreg_prime _))] => apply/disjoint_qregP_prime
  | [ |- is_true (disjoint_qreg (qreg_pair _ _) _)] => apply/disjoint_qreg_pairL
  | [ |- is_true (disjoint_qreg _ (qreg_pair _ _))] => apply/disjoint_qreg_pairR
  | [ |- is_true (disjoint_qreg (qreg_tuple _) _)] => apply/disjoint_qreg_tupleL
  | [ |- is_true (disjoint_qreg _ (qreg_tuple _))] => apply/disjoint_qreg_tupleR
  | [ |- is_true (disjoint_qreg (qreg_ffun _) _)] => apply/disjoint_qreg_ffunL
  | [ |- is_true (disjoint_qreg _ (qreg_ffun _))] => apply/disjoint_qreg_ffunR
...
Ltac move_extra := repeat match goal with
  | [ H : is_true (@eq_op nat_eqType _ _) |- _ ] => move : H; try clear H
  | [ H : is_true (negb (@eq_op nat_eqType _ _)) |- _ ] => move : H; try clear H
  | [ H : is_true (leq _ _) |- _ ] => move : H; try clear H
  | [ H : is_true (negb (leq _ _)) |- _ ] => move : H; try clear H
  | [ H : _ |- _ ] => try clear H; try move : H; try clear H
end.
...
Ltac tac_expose := rewrite /qreg_expose; intros;
  rewrite /= ?(eq_qreg_fst, eq_qreg_snd, eq_qreg_tuplei, eq_qreg_ffuni)/=;
    ...
    | [ |- is_true (negb (@eq_op qvar_eqType _ _ ))] => tac_qvar_diff
...
Goal forall m, valid_qreg (qsem_of <{{ (x.[(@half_ord _)%:F `i], x.[(@rev_ord _)%:F ((@half_ord _)%:F `i)]) }}> m).
Proof. time QRegAuto.tac_expose. Qed. *)
