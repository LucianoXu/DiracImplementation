...
Notation IhbType T m := (@pack T m _ _ idfun).
...
(* -------------------------------------------------------------------- *)
Definition witness_of (T : ihbType) & phant T : T := nosimpl
  (Inhabited.mixin (Inhabited.class T)).
...
Canonical ordinal_ihbType n := Eval hnf in IhbType 'I_n.+1 ord0.
Canonical bool_ihbType := Eval hnf in IhbType bool false.
...
(* -------------------------------------------------------------------- *)
Canonical option_ihbType (T : choiceType) :=
  Eval hnf in IhbType (option T) None.
Canonical prod_ihbType (T U : ihbType) :=
  Eval hnf in IhbType (T * U)%type (witness T, witness U).
Canonical sum_ihbType (T1 T2 : ihbType) :=
  Eval hnf in IhbType (T1 + T2) (inl (witness T1)).
...
Canonical ffun_ihbType (F : finType) (T : ihbType) :=
  Eval hnf in IhbType {ffun F -> T} [ffun=> witness T].
Canonical dffun_ihbType (F : finType) (T : forall i : F, ihbType) :=
  Eval hnf in IhbType {dffun forall i : F, T i} [ffun i => witness (T i)].
...
Definition base2 T (cT : class_of T) : Inhabited.class_of T :=
  @Inhabited.Class _ (@base T cT) (@mixin T cT).
...
Definition pack :=
  fun bT b & phant_id (Finite.class bT) (b : Finite.class_of T) =>
  fun mT m & phant_id (Inhabited.class mT) (@Inhabited.Class T b m) =>
  Pack (@Class T b m).

Definition eqType := @Equality.Pack cT class.
Definition choiceType := @Choice.Pack cT class.
Definition finType := @Finite.Pack cT class.
Definition ihbType := @Inhabited.Pack cT class.
End ClassDef.

Module Import Exports.
Coercion base : class_of >-> Finite.class_of.
Coercion mixin : class_of >-> Inhabited.mixin_of.
...
Canonical finType.
Coercion ihbType : type >-> Inhabited.type.
Canonical ihbType.
...
Notation "[ 'ihbFinType' 'of' T ]" := (@pack T _ _ id _ _ id)
...
(* -------------------------------------------------------------------- *)
Canonical unit_ihbFinType := Eval hnf in [ihbFinType of unit].
Canonical bool_ihbFinType := Eval hnf in [ihbFinType of bool].
Canonical ordinal_ihbFinType n := Eval hnf in [ihbFinType of 'I_n.+1].
Canonical option_ihbFinType (F: finType) := Eval hnf in [ihbFinType of (option F)].

(* -------------------------------------------------------------------- *)
Canonical prod_ihbFinType (T U : ihbFinType) :=
  Eval hnf in [ihbFinType of T * U].
Canonical sum_ihbFinType (T1 T2 : ihbFinType) :=
    Eval hnf in [ihbFinType of T1 + T2].
Canonical tuple_ihbFinType n (T : ihbFinType) :=
  Eval hnf in [ihbFinType of n.-tuple T].
Canonical ffun_ihbFinType (F : finType) (T : ihbFinType) :=
  Eval hnf in [ihbFinType of {ffun F -> T}].
Canonical dffun_ihbFinType (F : finType) (T : forall i : F, ihbFinType) :=
  Eval hnf in [ihbFinType of {dffun forall i : F, T i}].
...
Lemma ihb_v2hE : (@v2hU ihb_chsType) = 1%:M.
Proof.
apply/matrixP=>i j.
by rewrite/v2hU !mxE tnthE /eb/= r2vK mxE eqxx.
Qed.

Lemma ihb_h2vE : (@h2vU ihb_chsType) = 1%:M.
Proof. by rewrite/h2vU ihb_v2hE invmx1. Qed.

Lemma ihb_h2cE (v : ihb_chsType) : h2c v = ((v2r v)^T)%R.
Proof. by rewrite h2cE ihb_h2vE mul1mx. Qed.

Lemma ihb_c2hE v : (c2h v : ihb_chsType) = r2v v^T.
...
Proof. by apply/h2c_inj/matrixP=>i j; rewrite /mv2tv !c2hK !mxE. Qed.
...
Proof. by apply/h2c_inj/matrixP=>i j; rewrite !c2hK !mxE rmorphM. Qed.
...
Proof. by apply/h2mx_inj/matrixP=>i j; rewrite !mx2hK !mxE rmorphM. Qed.
...
Proof.
move=>/ge0_formV[f1 Pf] /ge0_formV[g1 Pg].
...
Lemma tentf00 : tentf 0 0 = (0 : 'Hom[T1 * T1, T2 * T4]).
...
Proof.
apply/h2c_inj/matrixP=>i j; rewrite !c2hK !mxE rmorph_prod;
...
Qed.
Lemma tentv_tuple_onb (G : finType) (f : 'I_n -> 'ONB(G;'Hs T)) i j :
  [< tentv_tuple_fun f i ; tentv_tuple_fun f j >] = (i == j)%:R.
Proof. exact: tentv_tuple_ponb. Qed.
Lemma tentv_tuple_card (G : finType) (f : 'I_n -> 'ONB(G;'Hs T)) : 
  #|{: n.-tuple G}| = Vector.dim ('Hs (n.-tuple T)).
Proof.
rewrite -ihb_dim_cast !card_tuple. case: n f; first by rewrite !expn0.
by move=>m f; rewrite (onb_card (f ord0)) ihb_dim_cast.
Qed.
Canonical tentv_tuple_ponbasis G f := PONBasis (@tentv_tuple_ponb G f).
Canonical tentv_tuple_onbasis G f := ONBasis (@tentv_tuple_onb G f) (tentv_tuple_card f).
Definition tentvtv_tuple_onbasis := tentv_tuple_onbasis (fun i=>t2tv_onbasis).
...
Proof.
by apply/h2mx_inj/matrixP=>i j; rewrite !mx2hK !mxE rmorph_prod;
...
Proof.
apply/h2mx_inj/matrixP=>i j; rewrite/= !mx2hK !mxE rmorph_prod; 
...
Proof.
apply/h2mx_inj/matrixP=>i j; rewrite/= !mx2hK !mxE; 
by apply eq_bigr => k _; rewrite mx2hK !mxE.
Qed.
...
Lemma permtf_conj (s : 'S_n) :
  (permtf s)^C = permtf s.
Proof. by apply/h2mx_inj/matrixP=>i j; rewrite !mx2hK !mxE conjC_nat. Qed.

Lemma permtf_tr (s : 'S_n) :
  (permtf s)^T = permtf (s^-1)%g.
Proof.
apply/h2mx_inj/matrixP=>i j; rewrite !mx2hK !mxE invgK.
...
Lemma tentf_tuple1 : tentf_tuple (fun i : 'I_n=> \1 : 'End[T]) = \1.
Proof.
apply/(intro_onb t2tv_onbasis)=>i.
...
Lemma tentf_tuple_ge0 (f : 'I_n -> 'End[T]) :
  (forall i, 0%:VF ⊑ f i) -> 0%:VF ⊑ tentf_tuple f.
Proof.
move=>P. have P1 i : {g : 'End[T] | f i = g^A \o g}.
by move: (P i)=>/ge0_formV/sig_eqW[g Pg]; exists g.
suff -> : tentf_tuple f = (tentf_tuple (fun i=>projT1 (P1 i)))^A \o
  (tentf_tuple (fun i=>projT1 (P1 i))) by apply form_ge0.
rewrite tentf_tuple_adj tentf_tuple_comp; apply eq_tentf_tuple=>i.
exact: (projT2 (P1 i)).
Qed.

Lemma tentf_tuple_lef (f g : 'I_n -> 'End[T]) :
  (forall i, 0%:VF ⊑ f i) -> (forall i, f i ⊑ g i) -> tentf_tuple f ⊑ tentf_tuple g.
Proof.
move=>P1 P2.
pose fn k := \mvector_(i : 'I_n) (if (i < k)%N then g i else f i).
have fn0 : tentf_tuple f = tentf_tuple (fn 0%N).
by apply eq_tentf_tuple=>i; rewrite mvE ltn0.
have fnn k : (n <= k)%N -> tentf_tuple g = tentf_tuple (fn k).
move=>P; apply eq_tentf_tuple=>i; rewrite mvE; case: leqP=>// P3.
by move: (leq_trans P P3); rewrite leqNgt ltn_ord.
suff: forall k, tentf_tuple (fn 0%N) ⊑ tentf_tuple (fn k).
move=>/(_ n); rewrite fn0 (fnn n)//.
move=>k; elim: k=>[//|k IH].
apply: (le_trans IH); case E: (k < n)%N; last first.
rewrite -!fnn ?lexx//. 2: apply/ltnW. 1,2: rewrite ?ltnS leqNgt E//.
suff P3 (t : 'I_n) : fn t.+1 = fn t +_ t (g t - f t).
move: (P3 (Ordinal E))=>/=->.
rewrite !tentf_tmvEV mlinearD. exact: tentf_tmv_mlinear.
rewrite levDl; apply/tentf_tuple_ge0=>i.
case E1: (i == Ordinal E).
move: E1=>/eqP E1; case: (Ordinal E) / E1.
...
apply/mvectorP=>i.
have P3 (x y : 'I_n) : (x < y)%N -> x != y /\ (x < y /\ x < y.+1)%N.
by move=>H; do ? split=>//; move: H; rewrite ltn_neqAle ?ltnS=>/andP[].
case: (ltngtP i t)=>[/P3|/P3|/val_inj P4];rewrite ?enum_rankK.
by move=>[]P4[]P5 P6; rewrite mvE mvE mset_ne 1?eq_sym// !mvE P5 P6 addr0.
by move=>[]P4[]P5 P6; rewrite mvE mvE mset_ne// !mvE ltnS 
  ltn_neqAle leqNgt P5/= eq_sym P4/= addr0.
by rewrite mvE P4 mvE msetii ltnSn mvE ltnn addrC addrNK.
Qed.
...
Qed.
Canonical tentf_tuple_hermfType f := HermfType (@tentf_tuple_herm f).
Lemma tentf_tuple_psd (f : 'I_n -> 'F+('Hs T)) : tentf_tuple f \is psdlf.
Proof. rewrite psdlfE; apply/tentf_tuple_ge0=>i; apply/psdf_ge0. Qed.
Canonical tentf_tuple_psdfType f := PsdfType (@tentf_tuple_psd f).
Lemma tentf_tuple_den (f : 'I_n -> 'FD('Hs T)) : tentf_tuple f \is denlf.
Proof.
apply/denlfP; split; first by apply/psdf_psd.
rewrite tentf_tuple_trlf.
have P1 : \prod_(i < n) 1 = 1 :> C by rewrite prodr_const expr1n.
by rewrite -{2}P1 ler_prod// =>i _; rewrite psdf_trlf denf_trlf.
Qed.
Canonical tentf_tuple_denfType f := DenfType (@tentf_tuple_den f).
Lemma tentf_tuple_obs (f : 'I_n -> 'FO('Hs T)) : tentf_tuple f \is obslf.
Proof.
apply/obslf_lefP; rewrite psdf_ge0; split=>//.
by rewrite -tentf_tuple1; apply tentf_tuple_lef=>i; rewrite ?psdf_ge0// obsf_le1.
Qed.
Canonical tentf_tuple_obsfType f := ObsfType (@tentf_tuple_obs f).
Lemma tentf_tuple_unitary (f : 'I_n -> 'FU('Hs T)) : tentf_tuple f \is unitarylf.
Proof.
apply/unitarylfP; rewrite tentf_tuple_adj tentf_tuple_comp.
by under eq_tentf_tuple do rewrite unitaryf_form; rewrite tentf_tuple1.
Qed.
Canonical tentf_tuple_unitaryfType f := UnitaryfType (@tentf_tuple_unitary f).
Lemma tentf_tuple_den1 (f : 'I_n -> 'FD1('Hs T)) : tentf_tuple f \is den1lf.
Proof.
apply/den1lfP; split; first by apply/psdf_psd.
by rewrite tentf_tuple_trlf big1// =>i _; rewrite den1f_trlf.
Qed.
Canonical tentf_tuple_den1fType f := Den1fType (@tentf_tuple_den1 f).
...
Qed.
Canonical tentf_tuple_projfType f := ProjfType (@tentf_tuple_proj f).
Lemma tentf_tuple_proj1 (f : 'I_n -> 'FP1('Hs T)) : tentf_tuple f \is proj1lf.
Proof. by apply/proj1lfP; rewrite projf_proj den1f_trlf. Qed.
Canonical tentf_tuple_proj1fType f := Proj1fType (@tentf_tuple_proj1 f).
