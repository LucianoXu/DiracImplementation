...
Lemma linearlfE (R : ringType) (U V : lmodType R) 
  (f : U -> V) (lf: linear f) : f = Linear lf.
Proof. by []. Qed.

Lemma scalarlfE (R : ringType) (U : lmodType R) 
  (f : U -> R) (lf: scalar f) : f = Linear lf.
Proof. by []. Qed.
...
Canonical fsetI_muloid := MulLaw (@fset0I I) (@fsetI0 I).
Canonical fsetU_addoid := AddLaw (@fsetIUl I) (@fsetIUr I).
End FSetMonoids.
...
Lemma index_enum_fset (I : choiceType) (A : {fset I}) :
  perm_eq (map val (index_enum (@fset_sub_finType I A)))
    (@enum_fset I A).
Proof.
have /perm_undup: map val (index_enum (@fset_sub_finType I A)) =i @enum_fset I A.
...
Lemma big_in_fsetE (T : Type) (idm : T) (op : Monoid.com_law idm) 
 ...
 \big[op/idm]_(i <- (index_enum (@fset_sub_finType I A))) g (fsval i) = \big[op/idm]_(i <- A) g i.
...
Lemma enum_fsetT_perm :
  perm_eq (enum [finType of [fset x | x : I]])
    (map (fun x => [` in_fsetT x]) (enum I)).
Proof.
have: (enum [finType of [fset x | x : I]]) =i (map (fun x => [` in_fsetT x]) (enum I)).
