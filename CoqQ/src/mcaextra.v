...
Lemma compact_cluster : exists y, cluster [filter of f] y.
Proof.
set F := [filter of f].
...
Lemma foo4 y N e : cluster [filter of f] y
  -> 0 < e -> { M | (N < M)%N & `|f M - y| < e}.
Proof.
rewrite /cluster/==>P1 egt0.
have P2: nbhs f (f @` [set n | (n > N)%N]) by exists N.+1=>//= t/= Pt; exists t.
...
Lemma bounded_compact_complete (F : set (set V)) : 
