...
Definition finZmodType_ihbType :=
  @Inhabited.Pack T 
  (Inhabited.Class ((@FinRing.Zmodule.class T)) 0).
End finZmodType_ihbFinType.
Coercion finZmodType_ihbType : finZmodType >-> Inhabited.type.
Canonical finZmodType_ihbType.
Definition finZmodType_ihbFinType (T : finZmodType) :=
  Eval hnf in [ihbFinType of T].
...
rewrite lt_neqAle andbA andbAC pih2 andbT; apply/andP; split; last first.
  by apply/eqP => hpi2; have := cos2_lt0 R; rewrite /intmul -hpi2 cpih ltxx.
...
Proof.
by have := pihalf_ge1; rewrite ler_pdivlMr// mul1r. 
Qed.

Lemma pi_gt1 : 1 < pi :> R.
Proof. by apply/(lt_le_trans _ pi_ge2) => //; rewrite (natrD _ 1 1) ltrDl. Qed.
