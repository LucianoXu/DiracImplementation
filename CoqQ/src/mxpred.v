...
From quantum.external Require Import mxred forms spectral.
...
Require Import notation mcextra.
(* -------------------------------------------------------------------- *)
...
(****************************************************************************)
(* Different from matrix.v in mathcomp, we use column vectors               *)
...
(* for extendd part, i.e., index out of minn m n)                          *)
...
Canonical conjmx_additive m n := Additive (@conjmx_is_antilinear m n).
...
Canonical adjmx_additive m n := Additive (@adjmx_is_antilinear m n).
...
Proof.
elim/row_ind : N =>[N|p x N IH]; first by rewrite /mxrank !eqxx/= big_ord0.
...
rewrite !linearD/= !mulmxDl !linearD/= !linearZ/= !linearZl/= !linearZ/==>->->.
...
Definition applyar_head := applyr_head.
Notation applyar := (@applyar_head _ _ _ tt).
...
by move/eqP: E=>->; rewrite normr0 mul0r mulrDr mulrN mulrCA mulrV ?unitfE 
  ?normr_eq0 ?F// mulr1 add0r addrNK mulrN lerNr oppr0 mulr0 mul0r expr2.
...
by rewrite mulrCA invrM// ?unitfE ?pnatr_eq0// [ `|a| * _]mulrA mulrV// 
  mulrA mulr1 {2}(splitr `|b|) opprD addrA addrN add0r mulNr.
rewrite addrC !mulNr subr_ge0 mulrA ler_pdivrMr.
by apply mulr_gt0=>//; rewrite normr_gt0 E.
rewrite mulrC mulrA expr2 ler_pdivrMr ?ltr0Sn// =>P1.
...
by rewrite -P0 !expr2 mulrA [_ * `|b| * _]mulrC mulrA -natrM -!mulrA 
  -ler_pdivlMl ?ltr0Sn// !mulrA mulVr ?unitfE ?pnatr_eq0// mul1r.
Qed.
...
apply lerXn2r; rewrite ?nnegrE// [X in _ <= X]ger0_norm.
...
by rewrite !expr1 mxE ler_normD.
Qed.

Lemma l2norm_triangle (A B : 'M[R]_(m,n)) : l2norm (A + B) <= l2norm A + l2norm B.
Proof.
rewrite -(ler_pXn2r (ltn0Sn 1))//; first by apply addr_ge0.
rewrite /l2norm sqrrD !sqrtCK.
apply (@le_trans _ _ (\sum_i (`|A i.1 i.2| + `|B i.1 i.2|) ^+ 2)).
by apply ler_sum=>i _; rewrite lerXn2r// ?nnegrE// ?addr_ge0// mxE ler_normD.
under eq_bigr do rewrite sqrrD. rewrite !big_split/= lerD2r lerD2l
  -mulr2n lerMn2r//= -[X in X <= _]ger0_norm ?sumr_ge0//; last first.
rewrite -(@ler_pXn2r _ 2)// ?nnegrE// ?mulr_ge0// ?sqrtC_ge0 ?sumr_ge0//; last first.
rewrite exprMn !sqrtCK; apply: (le_trans (Cauchy_Schwarz_C _ _ _ _)).
rewrite le_eqVlt; apply/orP; left; apply/eqP.
by congr (_ * _); under eq_bigr do rewrite normr_id.
all: by move=>? _; rewrite ?expr2 mulr_ge0.
Qed.
